...
ts1$VAR_0200_VAR_0237 <- as.factor(paste(ts1$VAR_0200,ts1$VAR_0237, sep="_"))
...
set.seed(10)
tme <- Sys.time()
xgb14 <- xgboost(data        = data.matrix(train[,varnames]),
                 label       = train$target,
                 params = param,
                 print.every.n = 100,
                 nrounds     = 20000,
...
pbPost(type = "note", title = "XGB14", body="Done.")
save(xgb14, file="xgb14.rda")

targetPred <- predict(xgb14, data.matrix(test[,varnames]))
submission <- data.frame(ID=test$ID, target=targetPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-02-2015-xgb14.csv", row.names=FALSE)
