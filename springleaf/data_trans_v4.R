...
set.seed(10)
tme <- Sys.time()
xgb11 <- xgboost(data        = data.matrix(train[,varnames]),
                label       = train$target,
                params = param,
                print.every.n = 100,
                nrounds     = 20000,
...
pbPost(type = "note", title = "XGB11", body="Done.")
save(xgb11, file="xgb11.rda")

targetPred <- predict(xgb11, data.matrix(test[,varnames]))
submission <- data.frame(ID=test$ID, target=targetPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-09-28-2015-xgb11.csv", row.names=FALSE)
