...
set.seed(10)
tme <- Sys.time()
xgb13 <- xgboost(data        = data.matrix(train[,varnames]),
                 label       = train$target,
                 params = param,
                 print.every.n = 100,
                 nrounds     = 20000,
...
pbPost(type = "note", title = "XGB13", body="Done.")
save(xgb13, file="xgb13.rda")

targetPred <- predict(xgb13, data.matrix(test[,varnames]))
submission <- data.frame(ID=test$ID, target=targetPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-01-2015-xgb13.csv", row.names=FALSE)


pred1 <- read.csv("submission-09-28-2015-xgb11.csv")
pred2 <- read.csv("submission-09-29-2015-xgb12.csv")
pred3 <- read.csv("submission-09-25-2015-xgb10.csv")
