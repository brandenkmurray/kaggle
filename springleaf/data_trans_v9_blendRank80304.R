...
xgb15names <- xgb15Imp$Feature[1:1000]

set.seed(10)
tme <- Sys.time()
xgb16 <- xgboost(data        = data.matrix(trainInd[,xgb15names]),
                 label       = trainInd$target,
                 params = param,
                 print.every.n = 100,
                 nrounds     = 10000,
                 # missing = -989989,
...
pbPost(type = "note", title = "XGB16", body="Done.")
save(xgb16, file="xgb16.rda")
...
cor(pred5$target, pred1$target)

blendPred <- (pred1$target + pred2$target + pred3$target + pred4$target + pred5$target) / 5
submission <- data.frame(ID=testId, target=blendPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-08-2015-v9_blended.csv", row.names=FALSE)
...
blendRank <- (pred1$rank + pred2$rank + pred3$rank + pred4$rank + pred5$rank) / 5
head(blendRank)
blendRankNorm <- blendRank / max(blendRank)

submission <- data.frame(ID=testId, target=blendRankNorm)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-08-2015-v9_blendedRank.csv", row.names=FALSE)
