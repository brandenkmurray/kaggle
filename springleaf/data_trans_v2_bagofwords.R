...
t1$VAR_0493 <- as.character(t1$VAR_0493)


corpus = Corpus(VectorSource(t1$VAR_0493))
...
inspect(frequencies[1000:1005,400:410])
...
sparse = removeSparseTerms(frequencies, 0.99985)
sparse

# Convert to a data frame

t1Sparse = as.data.frame(as.matrix(sparse))

# Make all variable names R-friendly

colnames(t1Sparse) = make.names(paste0("VAR_0493_",colnames(t1Sparse)))

# Add dependent variable

t1Sparse$y <- as.factor(paste0("X",t1$target))
...
# Convert -999 back to NA for roughfix
ts1[ts1==-999] <- NA
ts1 <- na.roughfix(ts1)
