...
set.seed(999)
t1 <- fread("train.csv", stringsAsFactors=FALSE)
s1 <- fread("test.csv", stringsAsFactors=FALSE)
...
ts1[nums][is.na(ts1[nums])] <- -989989
...
# STATES - VAR_
highCardVars <- c("VAR_0200","VAR_0241","VAR_0241_4", "VAR_0241_3", "VAR_0404","VAR_0493")

ts1$VAR_0200_VAR_0237 <- as.numeric(as.factor(paste(ts1$VAR_0200,ts1$VAR_0237, sep="_")))

ts1_zip_sub <- ts1[,c("VAR_0200_VAR_0237","VAR_0241")]
ts1$VAR_city_zip_cnt <- sqldf("SELECT zip_cnt 
                              FROM ts1_zip_sub a LEFT JOIN 
                              (SELECT VAR_0200_VAR_0237 as city, count(DISTINCT VAR_0241) as zip_cnt FROM ts1_zip_sub GROUP BY city) b
                              ON a.VAR_0200_VAR_0237=b.city")$zip_cnt
...
# Create Corpi
corpus0404 <- Corpus(VectorSource(ts1$VAR_0404))
corpus0493 <- Corpus(VectorSource(ts1$VAR_0493))
# Convert to lowercase
corpus0404 <- tm_map(corpus0404, tolower)
corpus0493 <- tm_map(corpus0493, tolower)
# Convert to PlainTextDocument
corpus0404 <- tm_map(corpus0404, PlainTextDocument)
corpus0493 <- tm_map(corpus0493, PlainTextDocument)
# Remove puncuation
corpus0404 <- tm_map(corpus0404, removePunctuation)
corpus0493 <- tm_map(corpus0493, removePunctuation)
# Remove any stopwords
corpus0404 <- tm_map(corpus0404, removeWords, stopwords("english"))
corpus0493 <- tm_map(corpus0493, removeWords, stopwords("english"))
# Stem
corpus0404 <- tm_map(corpus0404, stemDocument)
corpus0493 <- tm_map(corpus0493, stemDocument)
# Create word matrices
freq0404 <- DocumentTermMatrix(corpus0404)
freq0493 <- DocumentTermMatrix(corpus0493)
# See what terms are frequent
# findFreqTerms(freq0404, lowfreq=30)
# findFreqTerms(freq0493, lowfreq=30)
# Remove sparse terms -- set the sparsity parameter to keep about 100 terms from each
sparse0404 <- removeSparseTerms(freq0404, .99989)
sparse0493 <- removeSparseTerms(freq0493, .99985)
# Convert to data frame
df0404 <- as.data.frame(as.matrix(sparse0404))
df0493 <- as.data.frame(as.matrix(sparse0493))
# Rename columns so they have VAR in them
colnames(df0404) <- make.names(paste0("VAR_0404",colnames(df0404)))
colnames(df0493) <- make.names(paste0("VAR_0493",colnames(df0493)))
# Bind to ts1
ts1 <- cbind(ts1, df0404)
ts1 <- cbind(ts1, df0493)
# Remove bag of words objects
rm("freq0404","freq0493","corpus0404","corpus0493","sparse0404","sparse0493","df0404","df0493")
################################################################################################
...
ts1[is.na(ts1)] <- -989989
trainInd <- data.frame(model.matrix(target ~ ., data=ts1[ts1$filter==0, c("target",varnames)]))[,-1]
testInd <- data.frame(model.matrix(target ~ ., data=ts1[ts1$filter==2, c("target",varnames)]))[,-1]
trainInd$target <- ts1$target[ts1$filter==0]
testInd$target <- ts1$target[ts1$filter==2]
...
library(xgboost)
library(RPushbullet)
varnames <- names(trainInd[,grepl("VAR", names(trainInd))])
dtrain <- xgb.DMatrix(data=data.matrix(trainInd[,varnames]),label=trainInd$target)
grid <- expand.grid(eta=c(0.03), subsample=c(0.5,1), colsample_bytree=c(0.5,1), max_depth=c(8,12,16), nrounds=c(10000))
# grid <- expand.grid(eta=c(0.1), max_depth=c(6), nrounds=c(5,6))
...
for (i in 1:nrow(grid)){
  ...
  predtmp <- predict(xgbtmp, data.matrix(testInd[,varnames]))
  predFrame[i] <- predtmp
  colnames(predFrame)[i] <- paste0("eta",grid[i,"eta"],"dep",grid[i,"max_depth"],"rnd",grid[i,"nrounds"],"sub",grid[i,"subsample"],"bytree",grid[i,"colsample_bytree"])
  rm(xgbtmp)
  gc()
  print(Sys.time() - tme1)
}
(runTime <- Sys.time() - tme)
write.csv(predFrame, "predFrame-xgbLoop1.csv")
pbPost(type = "note", title = "XGBLoop", body="Done.")
save(xgbtmp, file="xgb16.rda")

targetPred <- rowMeans(predFrame)
submission <- data.frame(ID=testId, target=targetPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-15-2015-xgbLoop2.csv", row.names=FALSE)




targetPred <- predict(xgb15, data.matrix(testInd[,varnames]))
submission <- data.frame(ID=testId, target=targetPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-03-2015-xgb15.csv", row.names=FALSE)

xgb15Imp <- xgb.importance(feature_names = varnames, model=xgb15)

pred1 <- read.csv("submission-09-28-2015-xgb11.csv")
pred2 <- read.csv("submission-09-29-2015-xgb12.csv")
pred3 <- read.csv("submission-10-01-2015-xgb13.csv")

cor(targetPred, pred1$target)

blendPred <- (targetPred + pred1$target + pred2$target + pred3$target) / 4
submission <- data.frame(ID=test$ID, target=blendPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-02-2015-blendedPreds.csv", row.names=FALSE)
...
pred2 <- read.csv("submission-09-29-2015-xgb12.csv")
pred3 <- read.csv("submission-10-01-2015-xgb13.csv")
pred4 <- read.csv("submission-10-02-2015-xgb14.csv")
pred5 <- read.csv("submission-10-03-2015-xgb15.csv")
pred6 <- read.csv("submission-10-15-2015-xgbLoop2.csv")

blendPred <- (pred1$target*.18 + pred2$target*.18 + pred3$target*.18 + pred4$target*.18 + pred5$target*.18 + pred6*.1)
submission <- data.frame(ID=testId, target=blendPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-15-2015-v10_blended.csv", row.names=FALSE)

#.80309
blendPred <- (pred1$target*.16666666 + pred2$target*.16666666 + pred3$target*.16666666 + pred4$target*.16666666 + pred5$target*.16666666 + targetPred*.16666666)
submission <- data.frame(ID=testId, target=blendPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-15-2015-v10_blended2.csv", row.names=FALSE)
...
#.80304
blendPred <- (pred1$target*.2 + pred2$target*.27+ pred3$target*.27 + pred4$target*.15 + pred5$target*.07 + targetPred*.04)
submission <- data.frame(ID=testId, target=blendPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-15-2015-v10_blended3.csv", row.names=FALSE)



pred1$rank <- rank(pred1$target, ties.method="average")
pred2$rank <- rank(pred2$target, ties.method="average")
pred3$rank <- rank(pred3$target, ties.method="average")
pred4$rank <- rank(pred4$target, ties.method="average")
pred5$rank <- rank(pred5$target, ties.method="average")
pred6$rank <- rank(pred6$target, ties.method="average")


blendRank <- (pred1$rank + pred2$rank + pred3$rank + pred4$rank + pred5$rank + pred6$rank) / 6
head(blendRank)
blendRankNorm <- blendRank / max(blendRank)
#0.80311
submission <- data.frame(ID=testId, target=blendRankNorm)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-15-2015-v10_blendedRank.csv", row.names=FALSE)


#weighted Blended Rank
blendRank <- (.2*pred1$rank + .27*pred2$rank + .27*pred3$rank + .15*pred4$rank + .07*pred5$rank + 0.04*pred6$rank) / 6
head(blendRank)
blendRankNorm <- blendRank / max(blendRank)
...
targetPred <- predFrame[12]
submission <- data.frame(ID=testId, target=targetPred)
colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-15-2015-xgbLoopCol12.csv", row.names=FALSE)
