...
ts1$VAR_0241_4 <- substr(ts1$VAR_0241, 1, 4)
ts1$VAR_0241_3 <- substr(ts1$VAR_0241, 1, 3)
...
for (i in dateCombos) {
  days <- abs(as.numeric(difftime(time1 = ts1[,unlist(i)[1]], time2 = ts1[,unlist(i)[2]], units="days")))
  days[is.na(days)] <- -999
  ts1 <- cbind(ts1, days)
...
# # EVERY 2-WAY INTERACTION POSSIBLE -- OVERBOARD...
# highCardCombos <- combn(c(highCardVars,paste0(dates,"_yearmon"),paste0(dates,"_wday")), 2, simplify=FALSE)
# for(i in highCardCombos) {
#   print(i)
#   tmp <- cat2WayAvg(ts1, unlist(i)[1], unlist(i)[2],"target","prob0",filter=ts1$filter==0, k=20, f=10, r_k=0.1)
#   ts1 <- cbind(ts1, tmp)
...
ts1 <- ts1[,!names(ts1) %in% c(paste0(dates,"_yearmon"))]
ts1 <- ts1[,!names(ts1) %in% c(paste0(dates,"_wday"))]
...
gc()
# medJobs <- c("DOCTOR","MEDICAL","NURSE","NURSING","PHYSICIAN","THERAPIST","THERAPY","OPTOMETRIST","SURGEON","PEDIATRICS","EMT","PARAMEDIC","MEDIC","DENTAL","DENTIST")
# med <- grepl(paste(medJobs, collapse="|"), ts1$VAR_0493)
# tapply(ts1$target[ts1$filter==0], med[ts1$filter==0], mean, na.rm=TRUE)
# # 
# 
# nurseJobs <- c("NURSE","NURSING")
# nurse <- grepl(paste(nurseJobs, collapse="|"), ts1$VAR_0493)
# tapply(ts1$VAR_0490, nurse, mean, na.rm=TRUE)
# 
# nailJobs <- c("HAIR","COSMETOLOGY")
# nail <- grepl(paste(nailJobs, collapse="|"), ts1$VAR_0493)
# sum(nail)
# tapply(ts1$VAR_0490, nail, mean, na.rm=TRUE)
# 
# 
# # lawyer <- c("LAW","ATTORNEY","LEGAL")
# # law <- grepl(paste(lawyer, collapse="|"), ts1$VAR_0493)
# # tapply(ts1$target[ts1$filter==0], law[ts1$filter==0], mean, na.rm=TRUE)
# # 
# # reglic <- c("REGISTERED","LICENSED","CERTIFIED")
# # reglics <- grepl(paste(reglic, collapse="|"), ts1$VAR_0493)
# # tapply(ts1$target[ts1$filter==0], reglics[ts1$filter==0], mean, na.rm=TRUE)
# # 
# # 
# # sum(grepl(paste(c("REAL ESTATE"), collapse="|"), ts1$VAR_0493))
# # 
# # realestate <- c("REAL ESTATE")
# # restate <- grepl(paste(realestate, collapse="|"), ts1$VAR_0493)
# # tapply(ts1$target[ts1$filter==0], restate[ts1$filter==0], mean, na.rm=TRUE)
# # 
# # sales <- c("SALES")
# # salesman <- grepl(paste(sales, collapse="|"), ts1$VAR_0493)
# # tapply(ts1$target[ts1$filter==0], salesman[ts1$filter==0], mean, na.rm=TRUE)
# # 
# # ass <- c("ASSISTANT", "ASST")
# # assistant <- grepl(paste(ass, collapse="|"), ts1$VAR_0493)
# # tapply(ts1$target[ts1$filter==0], assistant[ts1$filter==0], mean, na.rm=TRUE)
# # 
# eng <- c("ENGINEER","ARCHITECT")
# engineer <- grepl(paste(eng, collapse="|"), ts1$VAR_0493)
# sum(engineer)
# tapply(ts1$VAR_0490, engineer, mean, na.rm=TRUE)
# 
# lab <- c("COSMET")
# labor <- grepl(paste(lab, collapse="|"), ts1$VAR_0493)
# sum(labor)
# tapply(ts1$VAR_0490, labor, mean, na.rm=TRUE)
# 
# sec <- c("SECURITY","GUARD")
# security <- grepl(paste(sec, collapse="|"), ts1$VAR_0493)
# tapply(ts1$VAR_0490, security, mean, na.rm=TRUE)
# 
# pres <- c("CEO","CHIEF EXEC")
# president <- grepl(paste(pres, collapse="|"), ts1$VAR_0404)
# sum(president)
# tapply(ts1$VAR_0490, president, mean, na.rm=TRUE)
# 
# levels(ts1$VAR_0493)[grepl("NURS", levels(ts1$VAR_0493))]
# # pharma <- c("PHARMA")
# # pharmacy <- grepl(paste(pharma, collapse="|"), ts1$VAR_0493)
# # tapply(ts1$target[ts1$filter==0], pharmacy[ts1$filter==0], mean, na.rm=TRUE)
# 
# jobBin <- grepl("-1", ts1$VAR_0404)
# tapply(ts1$VAR_0490, jobBin, mean, na.rm=TRUE)
# 
# ts1$jobBin <- jobBin
# ddply(ts1, .(jobBin), summarise, n=length(jobBin), mean490=mean(VAR_0490, na.rm=TRUE))
