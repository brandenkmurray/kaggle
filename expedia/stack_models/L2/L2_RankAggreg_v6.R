...
xgb7_loop1 <- fread("./stack_models/L1/testPreds/L1_testPreds_xgb7_loop1.csv", header=TRUE)
xgb8_loop1 <- fread("./stack_models/L1/testPreds/L1_testPreds_xgb8_loop1.csv", header=TRUE)
xgb8_loop2 <- fread("./stack_models/L1/testPreds/L1_testPreds_xgb8_loop2.csv", header=TRUE)
xgb8_loop3 <- fread("./stack_models/L1/testPreds/L1_testPreds_xgb8_loop3.csv", header=TRUE)
xgb8_loop4 <- fread("./stack_models/L1/testPreds/L1_testPreds_xgb8_loop4.csv", header=TRUE)
xgb8_loop5 <- fread("./stack_models/L1/testPreds/L1_testPreds_xgb8_loop5.csv", header=TRUE)
xgb8_loop6 <- fread("./stack_models/L1/testPreds/L1_testPreds_xgb8_loop6.csv", header=TRUE)
...
xgb7_loop1_split <- data.frame(do.call('rbind', strsplit(as.character(xgb7_loop1$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)
xgb8_loop1_split <- data.frame(do.call('rbind', strsplit(as.character(xgb8_loop1$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)
xgb8_loop2_split <- data.frame(do.call('rbind', strsplit(as.character(xgb8_loop2$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)
xgb8_loop3_split <- data.frame(do.call('rbind', strsplit(as.character(xgb8_loop3$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)
xgb8_loop4_split <- data.frame(do.call('rbind', strsplit(as.character(xgb8_loop4$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)
xgb8_loop5_split <- data.frame(do.call('rbind', strsplit(as.character(xgb8_loop5$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)
xgb8_loop6_split <- data.frame(do.call('rbind', strsplit(as.character(xgb8_loop6$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)
# locsplit <- data.frame(do.call('rbind', strsplit(as.character(bestloc$hotel_cluster),' ',fixed=TRUE)), stringsAsFactors = FALSE)

# 


w=matrix(rep(c(5,4,3,2,1),12), ncol=5, byrow=TRUE)
imp <- c(24,rep(2,11))
...
binded <- cbind(pub_split, xgb6_loop1_split, xgb6_loop2_split, xgb6_loop3_split, xgb6_loop4_split, xgb7_loop1_split, xgb8_loop1_split, xgb8_loop2_split, xgb8_loop3_split, xgb8_loop4_split, xgb8_loop5_split, xgb8_loop6_split)
...
# registerDoParallel(cl)
# raVec <- foreach(i=1:nrow(bestpub), .combine=c, .packages=c("data.table", "RankAggreg")) %dopar% {
#   paste(RankAggreg(matrix(as.character(binded[i,]), ncol=5, byrow=TRUE), k = 5, verbose=FALSE, seed=450+i)$top.list, collapse=" ")
#  }
# stopCluster(cl)


submission <- data.table(id=seq(0,nrow(bestpub)-1,1), hotel_cluster=raVec)
fwrite(submission, "./stack_models/L1/testPreds/L1_testPreds_RankAggreg_v6.csv")
