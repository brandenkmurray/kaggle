...
w=matrix(rep(c(5,4,3,2,1),11), ncol=5, byrow=TRUE)

cl <- makeCluster(14)
registerDoParallel(cl)
raVec <- foreach(i=1:nrow(bestloc), .combine=c, .packages=c("data.table", "RankAggreg")) %dopar% {
  paste(BruteAggreg(matrix(as.character(c(pub_split[i,], xgb16_split[i,], xgb17_split[i,], xgb18_split[i,], xgb19_split[i,], xgb20_split[i,], xgb21_split[i,], xgb1_loop_split[i,], xgb2_loop_split[i,], xgb3_loop_split[i,], xgb5_loop_split[i,])), ncol=5, byrow=TRUE), k = 5, weights=w, distance="Spearman", importance=c(5,rep(1,10)))$top.list, collapse=" ")
}
stopCluster(cl)

submission <- data.table(id=seq(0,nrow(bestloc)-1,1), hotel_cluster=raVec)
fwrite(submission, "./stack_models/L1/testPreds/L1_testPreds_RankAggreg_v2.csv")
