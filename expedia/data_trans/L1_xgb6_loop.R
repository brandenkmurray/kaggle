...
for (i in 1:loops){
    ...
    print(Sys.time())
    set.seed(2070+i)
    samp <- sample(1:sum(ts1$filter %in% c(0,1)), 2000000, replace=FALSE)
    ...
    fwrite(submission, paste0("./stack_models/L1/testPreds/L1_testPreds_xgb6_loop",i,".csv"))
    # fwrite(data.table(id=seq(0,nrow(testPreds)-1,1),testPreds), paste0("./stack_models/L1/testPreds/L1_testPreds_xgb6_loop",i,"_probs.csv"))
...
fwrite(submission, "./stack_models/L1/testPreds/L1_testPreds_xgb6_loop.csv")
fwrite(data.table(id=seq(0,nrow(testPreds_top5)-1,1),testPreds), "./stack_models/L1/testPreds/L1_testPreds_xgb6_loop_probs.csv")
