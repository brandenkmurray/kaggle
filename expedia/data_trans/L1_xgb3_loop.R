...
for (i in 1:loops){
    print(paste("loop ", i))
    set.seed(2030+i)
    samp <- sample(1:sum(ts1$filter %in% c(0,1) & ts1$date_time_year==2014), 1000000, replace=FALSE)
    dtrain <- xgb.DMatrix(data=data.matrix(ts1[filter %in% c(0,1) & date_time_year==2014,varnames, with=FALSE][samp]),label=data.matrix(ts1[filter %in% c(0,1) & date_time_year==2014, hotel_cluster][samp]))
    ...
    fwrite(data.table(id=seq(0,nrow(testPreds_top5)-1,1),testPreds), paste0("./stack_models/L1/testPreds/L1_testPreds_xgb3_loop",i,"_probs.csv"))
    
    testPredsMatrix <- testPredsMatrix + testPreds
}  
Sys.time() - tme   

testPreds <- testPredsMatrix/loops
colnames(testPreds) <- as.character(0:99)
...
fwrite(submission, "./stack_models/L1/testPreds/L1_testPreds_xgb3_loop.csv")
fwrite(data.table(id=seq(0,nrow(testPreds_top5)-1,1),testPreds), "./stack_models/L1/testPreds/L1_testPreds_xgb3_loop_probs.csv")
