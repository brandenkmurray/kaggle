...
ts1[,":="(dist_clust_bookings_sum=sum(is_booking[filter==0]),
          dist_clust_bookings_count=length(is_booking[filter==0]),
          dist_clust_cnts_sum=sum(cnt[filter==0]),
          dist_clust_cnts_count=length(cnt[filter==0])),
    keyby=list(orig_destination_distance)]
ts1[,":="(destid_clust_bookings_sum=sum(is_booking[filter==0]),
          destid_clust_bookings_count=length(is_booking[filter==0]),
          destid_clust_cnts_sum=sum(cnt[filter==0]),
          destid_clust_cnts_count=length(cnt[filter==0])),
...
leak <- fread("./data_trans/leak_props.csv", key="id2")
ts1[leak, (colnames(leak)[2:ncol(leak)]):=leak[,2:ncol(leak),with=FALSE]]; rm(leak); gc()
destid_clust <- fread("./data_trans/data_trans_destid_props.csv", key="id2")
...
library(xgboost)
dtrain <- xgb.DMatrix(data=data.matrix(ts1[1:31400000][filter==0,varnames, with=FALSE][samp]),label=data.matrix(ts1[1:31400000][filter==0, hotel_cluster][samp]))
dval <- xgb.DMatrix(data=data.matrix(ts1[-(1:31400000)][filter==0 & is_booking==1,varnames, with=FALSE]),label=data.matrix(ts1[-(1:31400000)][filter==0 & is_booking==1, hotel_cluster]))
...
fwrite(submission, "./stack_models/L1/testPreds/L1_testPreds_xgb1_v5.csv")
