...
# valrows<-nrow(ts1[-(1:31400000)][filter %in% c(0,1) & is_booking==1])
# set.seed(2018)
...
# cvFoldsList <-createFolds(ts1[filter %in% c(0,1), hotel_cluster][samp], k=3)
...
fwrite(submission, "./stack_models/L1/testPreds/L1_testPreds_xgb1_v19.csv")
fwrite(data.table(id=seq(0,nrow(testPreds_top5)-1,1),testPreds), "./stack_models/L1/testPreds/L1_testPreds_xgb1_v19_probs.csv")
