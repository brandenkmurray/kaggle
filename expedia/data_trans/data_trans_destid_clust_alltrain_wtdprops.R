...
srch_destid_clust_cast <- dcast.data.table(ts1[filter %in% c(0,1)], srch_destination_id ~ hotel_cluster, fun.aggregate=function(x) length(x)*.15 + sum(x)*.85, value.var="is_booking")
...
fwrite(srch_destid_clust_cast, "./data_trans/data_trans_destid_alltrain_wtdprops.csv")
