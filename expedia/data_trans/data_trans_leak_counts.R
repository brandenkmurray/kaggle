...
leak <- dcast.data.table(ts1[filter==0], user_location_country+user_location_region+user_location_city+hotel_market+orig_destination_distance ~ hotel_cluster, fun.aggregate=length, value.var="hotel_cluster")
...
clusterDum <- dummy(leak[filter==0,hotel_cluster])
# clusterDum <- clusterDum[,2:101]
for (col in 1:100) {
  set(leak, i=1:31400000, j=col+8L, value=leak[[col+8L]][1:31400000]-clusterDum[,col+0L])
}
...
fwrite(leak, "./data_trans/leak_counts.csv")
