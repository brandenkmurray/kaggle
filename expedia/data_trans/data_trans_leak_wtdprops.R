...
leak <- dcast.data.table(ts1[filter==0][1:31400000], user_location_country+user_location_region+user_location_city+hotel_market+orig_destination_distance ~ hotel_cluster, fun.aggregate=function(x) length(x)*.15 + sum(x)*.85, value.var="is_booking")
...
clusterDum <- dummy(leak[filter==0,hotel_cluster][1:31400000])
# clusterDum <- clusterDum[,2:101]
for (col in 1:100) {
  set(leak, i=1:31400000, j=col+8L, value=pmax(0,leak[[col+8L]][1:31400000]-clusterDum[,col+0L]))
}
...
fwrite(leak, "./data_trans/data_trans_leak_wtdprops.csv")
