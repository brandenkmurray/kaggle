...
# Create lag feature, From Zanzibar Lector in forums
t1 <- t1[order(t1$Store,t1$Date),]
uniq_stores <- unique(t1$Store)
t1$ClosedPrevious <- NA
coll.df <- data.frame()
for (i in uniq_stores) { 
  temp <- subset(data.table(t1), Store == i) 
  # create a separate column with lagged values 
  temp[, Open_Shift := shift(Open, 1L, type = 'lag')] 
  # create new column identifying stores closed on previous day 
  temp[, Closed_Previous := ifelse(Open == 1 & Open_Shift == 0, 1, 0)] 
  coll.df <- rbind(coll.df, temp) 
  rm(temp)
}
t1$ClosedPrevious <- coll.df$ClosedPrevious

s1 <- s1[order(s1$Store,s1$Date),]
uniq_stores <- unique(s1$Store)
s1$ClosedPrevious <- NA
coll.df <- data.frame()
for (i in uniq_stores) { 
  temp <- subset(data.table(s1), Store == i) 
  # create a separate column with lagged values 
  temp[, Open_Shift := shift(Open, 1L, type = 'lag')] 
  # create new column identifying stores closed on previous day 
  temp[, Closed_Previous := ifelse(Open == 1 & Open_Shift == 0, 1, 0)] 
...
s1$ClosedPrevious <- coll.df$ClosedPrevious
...
ts1$Date_mon <- strptime(ts1$Date, "%Y-%m-%d")$mon + 1
...
addDimTrain <- factorToNumeric(train, train, "Sales", "Store", metric=c("mean","median","sd","skewness","kurtosis"))
addDimTest <- factorToNumeric(train, test, "Sales", "Store", metric=c("mean","median","sd","skewness","kurtosis"))
