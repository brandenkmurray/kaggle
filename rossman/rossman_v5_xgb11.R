...
salesPred <- exp(predict(xgb10, data.matrix(test[,varnames])))-1
submission <- data.frame(Id=s1ID, Sales=salesPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-23-2015-xgb10.csv", row.names=FALSE)
