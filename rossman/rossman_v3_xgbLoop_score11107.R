...
for (i in 1:nrow(grid)){
      ...
      predtmp <- exp(predict(xgbtmp, data.matrix(testInd[,varnames])))-1
      predFrame[i] <- predtmp
...
(runTime <- Sys.time() - tme)
write.csv(predFrame, "predFrame.csv")

salesPred <- rowMeans(predFrame)
submission <- data.frame(Id=s1ID, Sales=salesPred)
# colnames(submission)[2] <- "target"
write.csv(submission, "submission-10-07-2015-xgbLoop.csv", row.names=FALSE)
