...
param <- list(  objective           = RMPSE_obj, 
                booster             = "gbtree",
                eta                 = 0.02, # 0.06, #0.01,
...
clf <- xgb.train(   params              = param, 
                    data                = dtrain, 
                    nrounds             = 10000, #300, #280, #125, #250, # changed from 300
                    verbose             = 0,
                    print.every.n       = 100,
...
pred1 <- exp(predict(clf, data.matrix(test[,varnames]))) -1
submission <- data.frame(Id=test$Id, Sales=pred1)
cat("saving the submission file\n")
write_csv(submission, "xgbLBScript-10-24-2015_10000rounds.csv")
