...
etLay2_v1 <- read_csv("./stack_models/cvPreds_lay2_et1.csv")

nnLay2_v1$VisitNumber <- NULL
etLay2_v1$VisitNumber <- NULL
...
logLossTable <- data.frame(x, LogLoss=rep(0, length(x))

i=1
for (p in 1:nrow(xComb)){
  tmp <- etLay2_v1*xComb[p,1] + xgbLay2_v16$pred*xComb[p,2] + nnLay2_v1*xComb[p,3]
...
etLay2_v1_fullpreds <- read_csv("./stack_models/xgbLay2_v16preds-et.csv")
...
etLay2_v1_fullpreds$VisitNumber <- NULL
...
blendedPreds <- xgbLay2_v16_fullpreds*.65 + nnLay2_v1_fullpreds*.35 + etLay2_v1_fullpreds*0
...
write.csv(submission, "./stack_models/blendedPreds_v6.csv", row.names=FALSE)
