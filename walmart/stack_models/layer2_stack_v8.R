...
lay1preds <- cbind(xgb1preds, xgb2preds, xgb3preds, kknn1preds, nn1preds)
...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v8 <- xgb.train(data = dtrain,
               params = param,
               nrounds = 6000,
               maximize=FALSE,
               watchlist=watchlist,
...
lay1fullpreds <- cbind(xgb1fullpreds, xgb2fullpreds, xgb3fullpreds, kknn1fullpreds, nn1fullpreds)
...
lay2preds <- predict(xgbLay2_v8, newdata=data.matrix(lay1fullpreds))
...
write.csv(submission, "./stack_models/xgbLay2_v8_preds.csv", row.names=FALSE)
