...
cnames <- paste("xgb4", names(samp)[2:ncol(samp)], sep="_")
colnames(xgb4$pred) <- cnames
write.csv(data.frame(VisitNumber=ts1Trans[filter==0,"VisitNumber",with=FALSE], xgb4$pred), "./stack_models/cvPreds_xgb4.csv", row.names=FALSE) 
...
preds <- predict(xgb4full, data.matrix(ts1Trans[filter==2, varnames, with=FALSE]))
...
write.csv(submission, "./stack_models/testPreds_xgb4full.csv", row.names=FALSE)
