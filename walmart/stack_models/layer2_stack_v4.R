...
lay1preds <- cbind(xgb1preds, xgb2preds, xgb3preds, kknn1preds, kknn2preds)
...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v4 <- xgb.train(data = dtrain,
               params = param,
               nrounds = 6000,
               maximize=FALSE,
               watchlist=watchlist,
...
lay1fullpreds <- cbind(xgb1fullpreds, xgb2fullpreds, xgb3fullpreds, kknn1fullpreds, kknn2fullpreds)
# Predict the test set using the XGBOOST stacked model
lay2preds <- predict(xgbLay2_v4, newdata=data.matrix(lay1fullpreds))
...
write.csv(submission, "./stack_models/xgbLay2_v4_preds.csv", row.names=FALSE)
