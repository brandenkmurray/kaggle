...
colnames(xgb7$pred) <- cnames
write.csv(data.frame(VisitNumber=ts1Trans[filter==0,"VisitNumber",with=FALSE], xgb7$pred), "./stack_models/cvPreds_xgb7.csv", row.names=FALSE) 

minLossRound <- which.min(xgb7$dt$test.mlogloss.mean)
rounds <- minLossRound * 1.1
...
preds <- predict(xgb7full, data.matrix(ts1Trans[filter==2, varnames, with=FALSE]))
...
write.csv(submission, "./stack_models/testPreds_xgb7full.csv", row.names=FALSE)
