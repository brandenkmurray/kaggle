...
lay1preds <- cbind(xgb1preds, xgb2preds, xgb3preds, xgb7preds, kknn1preds, kknn2preds, nn1preds)
...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v9 <- xgb.train(data = dtrain,
               params = param,
               nrounds = 6000,
               maximize=FALSE,
               watchlist=watchlist,
...
lay1fullpreds <- cbind(xgb1fullpreds, xgb2fullpreds, xgb3fullpreds, xgb7fullpreds, kknn1fullpreds, kknn2fullpreds, nn1fullpreds)
...
lay2preds <- predict(xgbLay2_v9, newdata=data.matrix(lay1fullpreds))
...
write.csv(submission, "./stack_models/xgbLay2_v9_preds.csv", row.names=FALSE)
