...
topFine <- x3$V1[which(rowMax>.50)]

topFine2 <- names(sort(table(t1$FinelineNumber), decreasing=TRUE))[1:700]
...
topUPC2 <- names(sort(table(t1$Upc), decreasing=TRUE))[1:200]
...
cvFolds <- createFolds(ts1Trans$class[ts1Trans$filter==0], k=5, list=TRUE, returnTrain=FALSE)
dval<-xgb.DMatrix(data=data.matrix(train[cvFolds[[5]],2:1145, with=FALSE]),label=data.matrix(train$class[cvFolds[[5]]]))
dtrain<-xgb.DMatrix(data=data.matrix(train[-cvFolds[[5]],2:1145, with=FALSE]),label=data.matrix(train$class[-cvFolds[[5]]]))
watchlist<-list(val=dval,train=dtrain)

classwts <- c(table(as.factor(make.names(train$class[-cvFolds[[5]]]))))
...
n_proc = detectCores()
tme <- Sys.time()
ext1 <- extraTrees(x=data.matrix(train[-cvFolds[[5]],varnames, with=FALSE]) ,
                   y=as.factor(make.names(train$class[-cvFolds[[5]]])),
                   ntree=2000,
                   mtry=16,
                   nodesize=1,
                   numRandomCuts=40,
                   numThreads=8)
Sys.time() - tme
.jcache(ext1$classifier)
save(ext1, file="ext2.rda")
extPreds <- predict(ext1, newdata=data.matrix(train[cvFolds[[5]],varnames, with=FALSE]),probability=T)
extPreds <- extPreds[,mixedorder(colnames(extPreds))]

extAct <- data.frame(extact=factor(make.names(train[cvFolds[[5]],class]),levels=c(levels(as.factor(make.names(tripClasses$class) )))))
extActTab <- model.matrix(~extact-1, extAct)
extActTab <- extActTab[,mixedorder(colnames(extActTab))]

LogLoss(extActTab, extPreds)



library(randomForest)
rf1 <- randomForest(x=data.matrix(train[-cvFolds[[5]],varnames, with=FALSE]) ,
                    y=as.factor(make.names(train$class[-cvFolds[[5]]])),
                    ntree=100,
                    mtry=8,
                    classwts=classwts,
                    ...
                    nodesize=1,
                    do.trace=5)
rfPreds <- predict(rf1, newdata=data.matrix(train[cvFolds[[5]],2:114,with=FALSE]),type="prob")
rfPreds <- rfPreds[,mixedorder(colnames(rfPreds))]

rfAct <- data.frame(rfact=factor(make.names(train[cvFolds[[5]],class]),levels=c(levels(as.factor(make.names(tripClasses$class) )))))
rfActTab <- model.matrix(~rfact-1, rfAct)
rfActTab <- rfActTab[,mixedorder(colnames(rfActTab))]

LogLoss(rfActTab, rfPreds)
...
preds <- predict(xgb7, data.matrix(train[cvFolds[[5]],2:114, with=FALSE]))
preds <- data.frame(t(matrix(preds, nrow=38, ncol=length(preds)/38)))
rfAct <- data.frame(rfact=factor(make.names(train[cvFolds[[5]],class]),levels=c(levels(as.factor(make.names(tripClasses$class) )))))
rfActTab <- model.matrix(~rfact-1, rfAct)
rfActTab <- rfActTab[,mixedorder(colnames(rfActTab))]

LogLoss(rfActTab, preds)



samp <- read.csv('sample_submission.csv')
cnames <- paste("xgb7", names(samp)[2:ncol(samp)], sep="_")
colnames(xgb6$pred) <- cnames
write.csv(data.frame(VisitNumber=ts1Trans[filter==0,"VisitNumber",with=FALSE], xgb6$pred), "./stack_models/cvPreds_xgb7.csv", row.names=FALSE) 
...
preds <- predict(xgb6full, data.matrix(ts1Trans[filter==2, varnames, with=FALSE]))
preds <- data.frame(t(matrix(preds, nrow=38, ncol=length(preds)/38)))
samp <- read.csv('sample_submission.csv')
cnames <- names(samp)[2:ncol(samp)]
names(preds) <- cnames
submission <- data.frame(VisitNumber=samp$VisitNumber, preds)
write.csv(submission, "./stack_models/testPreds_xgb6full.csv", row.names=FALSE)
