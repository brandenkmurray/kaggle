...
cnames <- paste("xgb6", names(samp)[2:ncol(samp)], sep="_")
colnames(xgb6$pred) <- cnames
write.csv(data.frame(VisitNumber=ts1Trans[filter==0,"VisitNumber",with=FALSE], xgb6$pred), "./stack_models/cvPreds_xgb6.csv", row.names=FALSE) 
