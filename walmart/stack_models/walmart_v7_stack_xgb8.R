...
cnames <- paste("xgb8", names(samp)[2:ncol(samp)], sep="_")
colnames(xgb8$pred) <- cnames
write.csv(data.frame(VisitNumber=ts1Trans[filter==0,"VisitNumber",with=FALSE], xgb8$pred), "./stack_models/cvPreds_xgb8.csv", row.names=FALSE) 

minLossRound <- which.min(xgb8$dt$test.mlogloss.mean)
...
preds <- predict(xgb8full, data.matrix(ts1Trans[filter==2, varnames, with=FALSE]))
...
write.csv(submission, "./stack_models/testPreds_xgb8full.csv", row.names=FALSE)
