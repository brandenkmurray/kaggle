...
kknn1_stack_preds <- matrix(0, nrow=nrow(t1kknn), ncol=38)
logLossTable <- data.frame(fold=seq(1:length(cvFolds)), LogLoss=rep(0, length(cvFolds)))
for (i in 1:length(cvFolds)){
  kknn1 <- kknn(as.factor(class) ~ .,
                train=t1kknn[cvFolds[[i]],], 
                test=t1kknn[-cvFolds[[i]],], 
                k=200, 
                distance=1,
                kernel="triweight")
  kknn1_stack_preds[-cvFolds[[i]],] <- kknn1$prob
  logLossTable[i,2] <- LogLoss(model.matrix(~class-1, t1kknn[-cvFolds[[i]],"class", with=FALSE]), kknn1$prob)
}

cvPreds <- kknn1_stack_preds
...
write.csv(cvPreds, "./stack_models/cvPreds_kknn1.csv", row.names=FALSE)

# LogLoss(model.matrix(~class-1, t1kknn[-cvFolds[[5]],"class", with=FALSE]), kknn1_stack_preds$prob)
LogLoss(model.matrix(~class-1, t1kknn[,"class", with=FALSE]), kknn1_stack_preds)


kknn1full <- kknn(as.factor(class) ~ .,
              train=t1kknn, 
              test=s1kknn, 
              k=200, 
              distance=1,
...
testPreds <- kknn1full$prob
colnames(testPreds) <- cnames
testPreds <- cbind(ts1Trans[filter==2,"VisitNumber",with=FALSE], testPreds)
write.csv(testPreds, "./stack_models/testPreds_kknn1full.csv", row.names=FALSE)
