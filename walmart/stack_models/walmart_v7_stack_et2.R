...
et2_stack_preds <- as.data.frame(matrix(0, nrow=nrow(train), ncol=38)) 
colnames(et2_stack_preds) <- unique(train$class)
et2_stack_preds <- et2_stack_preds[,mixedorder(colnames(et2_stack_preds))]
logLossTable <- data.frame(fold=seq(1:length(cvFoldsList)), LogLoss=rep(0, length(cvFoldsList)))
(tme <- Sys.time())
for (i in 1:length(cvFoldsList)){
  set.seed(201510)
  et2 <- extraTrees(x=data.matrix(train[-cvFoldsList[[1]],varnames, with=FALSE]),
                  y=train$class[-cvFoldsList[[1]]],
                  ntree=1500,
                  mtry=10,
                  nodesize=20,
                  numRandomCuts=8,
   ...
   et2_pred <- predict(et2, newdata=data.matrix(train[cvFoldsList[[1]],varnames, with=FALSE]), probability=TRUE)
    LogLoss(model.matrix(~as.factor(class)-1, train[cvFoldsList[[1]],"class", with=FALSE]), et2_pred)
#   
  # et2_stack_preds[cvFoldsList[[i]],] <- predict(et2, newdata=data.matrix(train[cvFoldsList[[i]],varnames, with=FALSE]), probability=TRUE)
  tmp <- as.data.frame(predict(et2, newdata=data.matrix(train[cvFoldsList[[i]],varnames, with=FALSE]), probability=TRUE))
  tmp <- tmp[,mixedorder(colnames(tmp))]
  et2_stack_preds[cvFoldsList[[i]],colnames(tmp)] <- tmp
  actTmp <- as.data.frame(model.matrix(~as.factor(class)-1, train[cvFoldsList[[i]],"class", with=FALSE]))
  actTmp <- actTmp[,mixedorder(colnames(actTmp))]
  logLossTable[i,2] <- LogLoss(actTmp, et2_stack_preds[cvFoldsList[[i]],])
}
Sys.time() - tme
logLossTable
...
cnames <- paste("et2", names(samp)[2:ncol(samp)], sep="_")
colnames(cvPreds) <- cnames
cvPreds <- cbind(ts1Trans[filter==0,"VisitNumber",with=FALSE], cvPreds)
write.csv(cvPreds, "./stack_models/cvPreds_et2.csv", row.names=FALSE)
...
Sys.time() - tme
prepareForSave(et2full)
save(et2full, file="./stack_models/et2full.rda")

preds <- predict(et2full, data.matrix(test[,varnames, with=FALSE]), probability=TRUE)
...
write.csv(submission, "./stack_models/testPreds_et2full.csv", row.names=FALSE)
