...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v6 <- xgb.train(data = dtrain,
               params = param,
               nrounds = 6000,
               maximize=FALSE,
               watchlist=watchlist,
...
lay1fullpreds <- cbind(#xgb1fullpreds, 
                       #xgb2fullpreds, 
                       xgb3fullpreds, 
                       xgb4fullpreds, 
                       xgb5fullpreds,
...
lay2preds <- predict(xgbLay2_v6, newdata=data.matrix(lay1fullpreds))
...
write.csv(submission, "./stack_models/xgbLay2_v6_preds.csv", row.names=FALSE)
## Scores
## v5 - 0.59299
## v6 - .059983