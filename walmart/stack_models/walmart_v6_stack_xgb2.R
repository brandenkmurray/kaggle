...
cnames <- paste("xgb2", names(samp)[2:ncol(samp)], sep="_")
colnames(xgb2$pred) <- cnames
write.csv(data.frame(VisitNumber=ts1Trans[filter==0,"VisitNumber",with=FALSE], xgb2$pred), "./stack_models/cvPreds_xgb2.csv", row.names=FALSE) 
...
preds <- predict(xgb2full, data.matrix(ts1Trans[filter==2, varnames, with=FALSE]))
...
write.csv(submission, "./stack_models/testPreds_xgb2full.csv", row.names=FALSE)
