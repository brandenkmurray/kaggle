...
cnames <- paste("xgb5", names(samp)[2:ncol(samp)], sep="_")
colnames(xgb5$pred) <- cnames
write.csv(data.frame(VisitNumber=ts1Trans[filter==0,"VisitNumber",with=FALSE], xgb5$pred), "./stack_models/cvPreds_xgb5.csv", row.names=FALSE) 
...
preds <- predict(xgb5full, data.matrix(ts1Trans[filter==2, varnames, with=FALSE]))
...
write.csv(submission, "./stack_models/testPreds_xgb5full.csv", row.names=FALSE)
