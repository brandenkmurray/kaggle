...
varnames <- colnames(data.matrix(t1Trans[h,3:ncol(t1Trans), with=FALSE]))
xgb3Imp <- xgb.importance(feature_names = varnames, model=xgb3)


preds <- predict(xgb3, data.matrix(s1Trans))
...
submission <- data.frame(VisitNumber=s1visit, predsMat)
write.csv(submission, "submit-xgb3-10-31-2015-test2.csv", row.names=FALSE)
