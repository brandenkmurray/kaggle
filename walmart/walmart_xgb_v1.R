...
data_transform <- function(data){
  ...
  if ("class" %in% colnames(data)) 
      {x <-data %>% 
        group_by(VisitNumber, class, Weekday) %>% 
        ...
        summarise(n=length(VisitNumber),
                  ...
                  scansFineRatio=netScans/uniqFine,
                  scansUpcRatio=netScans/uniqUpc)}
  xRes <- dcast(VisitNumber~DepartmentDescription, value.var="ScanCount",
                 fun.aggregate = function(x) sum(x), data=data)
...
dval<-xgb.DMatrix(data=data.matrix(t1Trans[h,3:ncol(t1Trans)]),label=t1Trans[h,"class"])
dtrain<-xgb.DMatrix(data=data.matrix(t1Trans[-h,3:ncol(t1Trans)]),label=t1Trans[-h,"class"])
...
varnames <- names(t1Trans[,3:ncol(t1Trans)])
xgb2Imp <- xgb.importance(feature_names = varnames, model=xgb2)
