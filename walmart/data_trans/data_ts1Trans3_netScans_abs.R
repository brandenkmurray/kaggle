...
ts1$netScans <- abs(ts1$ScanCount)
...
data_transform <- function(data){
  ...
  xWeekday <- dcast.data.table(VisitNumber~Weekday, value.var="netScans",
...
ts1Trans3 <- data_transform(ts1)
# Reorder the data set so that the train set is at the top and then order by VisitNumber
ts1Trans3 <- ts1Trans3[order(filter, VisitNumber),]
# Some entropy values were 0. This created some NAs for the entropy ratios
ts1Trans3[is.na(ts1Trans3)] <- 0

ts1Trans3[ , TripType38_helper := sum(`GROCERY DRY GOODS`, DAIRY, `COMM BREAD`,`PRE PACKED DELI`, na.rm=TRUE), by=1:NROW(ts1Trans3)]
ts1Trans3[ , TripType39_helper := sum(`PETS AND SUPPLIES`  ,`PERSONAL CARE`  ,`HOUSEHOLD CHEMICALS/SUPP`  ,BEAUTY  ,`PHARMACY OTC`, na.rm=TRUE), by=1:NROW(ts1Trans3)]
ts1Trans3[ , TripType7_helper := sum(BAKERY  ,`COMM BREAD`  ,DAIRY  ,`DSD GROCERY`  ,`FROZEN FOODS`  ,`GROCERY DRY GOODS`  ,`MEAT - FRESH & FROZEN`  ,`PRE PACKED DELI`  ,PRODUCE    ,`SERVICE DELI`, na.rm=TRUE), by=1:NROW(ts1Trans3)]
ts1Trans3[ , TripType8_helper := sum(DAIRY  ,`DSD GROCERY`  ,`PERSONAL CARE`  ,BEAUTY  ,`GROCERY DRY GOODS`  ,`IMPULSE MERCHANDISE`  ,PRODUCE, na.rm=TRUE), by=1:NROW(ts1Trans3)]
ts1Trans3[ , TripType9_helper := sum(AUTOMOTIVE  ,CELEBRATION  ,`MENS WEAR`  ,`OFFICE SUPPLIES`, na.rm=TRUE), by=1:NROW(ts1Trans3)]
ts1Trans3[ , TripType35_helper := sum(`CANDY, TOBACCO, COOKIES`  ,`DSD GROCERY`  ,`IMPULSE MERCHANDISE`, na.rm=TRUE), by=1:NROW(ts1Trans3)]
ts1Trans3[ , TripType36_helper := sum(BEAUTY  ,`PERSONAL CARE`  ,`PHARMACY OTC`  ,`PETS AND SUPPLIES`, na.rm=TRUE), by=1:NROW(ts1Trans3)]


write_csv(ts1Trans3, "./data_trans/ts1Trans3_netScans_abs.csv")


# Department distance/similarity
# Use only train data to get similarities
ts1_dept <- as.matrix(ts1Trans3[filter==0, 47:115, with=FALSE], nrow=nrow(ts1Trans))
...
# Use only train data to get similarities
ts1_fine <- as.matrix(ts1Trans3[filter==0, 116:5469, with=FALSE], nrow=nrow(ts1Trans))
...
ts1Trans3[, 47:115] <- ts1_dept_simil
ts1Trans3[, 116:5469] <- ts1_fine_simil

write_csv(ts1Trans3, "./data_trans/ts1Trans3_netScans_abs_simil.csv")
