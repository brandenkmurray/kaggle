...
write_csv(ts1Trans3, "./data_trans/ts1Trans3_prop.csv")

# Similarity matrices
# Use only train data to get similarities
ts1_dept <- as.matrix(ts1Trans3[, 47:115, with=FALSE])
...
# Use only train data to get similarities
ts1_fine <- as.matrix(ts1Trans3[, 116:5469, with=FALSE])
...
ts1Trans3[, colnames(ts1_dept_simil)] <- ts1_dept_simil
ts1Trans3[, colnames(ts1_fine_simil)] <- ts1_fine_simil

write_csv(ts1Trans3, "./data_trans/ts1Trans3_prop_simil.csv")
