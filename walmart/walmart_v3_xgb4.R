...
xgb4 <- xgb.train(data = dtrain,
                  ...
                  watchlist=watchlist,
                  early.stop.round=50)
save(xgb4, file="xgb4.rda")

xgb4Imp <- xgb.importance(feature_names = varnames, model=xgb4)

write.csv(xgb4Imp, "xgb4Imp.csv")
...
preds <- predict(xgb4, data.matrix(s1_new))
...
write.csv(submission, "submit-xgb4-11-1-2015.csv", row.names=FALSE)
