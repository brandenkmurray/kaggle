...
tme <- Sys.time()
set.seed(920)
gbmTrain <- train(outcome ~ . - freqIP - freqCountry - freqCountry2 , 
                  data=training,
                  method="gbm",
                  metric="ROC",
                  trControl=gbmCtrl,
...
rfCtrl <- trainControl(method="cv",
                        ...
                        allowParallel=TRUE,
                        summaryFunction=fiveStats)
rfGrid <- expand.grid(mtry=c(1,7,15,21,27))
...
gbmGrid <- expand.grid(n.trees=c(2500), interaction.depth=c(5,7,9,11,13), shrinkage=c(.001))
...
## Create my own ensemble  
rfPerc <- .5
ens_pred <- rfTest*rfPerc + gbmTest*(1-rfPerc)
