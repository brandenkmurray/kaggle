...
library(sqldf)
library(DMwR)
setwd("C:/Users/bmurray/My Documents/kaggle/bnp")
# Load data
...
s1 <- s1[,target:=-1]
# Combine into 1 data frame -- add new columns to beginning
l <- list(t1, s1)
ts1 <- data.table(do.call(smartbind,l))
ts1 <- cbind(pred0=mean(t1$target), dummy="A", filter=c(rep(x = 0, nrow(t1)), rep(2, nrow(s1))), ts1)

excludeCols <- c("ID","target","filter","dummy","pred0")

charCols <- which(sapply(ts1[,6:ncol(ts1),with=FALSE], is.character))
ts1_miss <- copy(ts1)
for (col in charCols){
...
ts1_miss <- ts1_miss[,6:ncol(ts1_miss),with=FALSE]
# K-Means Cluster
ts1_kmeans <- kmeans(ts1_miss, centers=7)


# Count NAs by row
ts1$clust <- ts1_kmeans$cluster
...
numCols <- colnames(ts1[,6:ncol(ts1),with=FALSE])[sapply(ts1[,6:ncol(ts1),with=FALSE], is.numeric)]
featCor <- cor(ts1[,numCols,with=FALSE])
hc <- findCorrelation(featCor, cutoff=0.999999, names=TRUE)
hc <- sort(hc)
ts1 <- ts1[,!colnames(ts1) %in% hc,with=FALSE]
...
pp <- preProcess(ts1[filter==0,6:ncol(ts1),with=FALSE], method=c("zv","center","scale","medianImpute"))
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v4.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v4.rda")

ts1_dum <- ts1_dum[order(ts1_dum$filter, ts1_dum$ID),]
write_csv(ts1_dum, "./data_trans/ts1Trans_v4.csv")
