...
hc <- sort(hc)
save(featCor, file="./data_trans/featCor_v29.rda")
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v29.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v29.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v29.csv")
