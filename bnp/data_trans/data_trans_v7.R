...
# }
# 
# save(imp_merged, file="./data_trans/mice_v7.rda")
# ts1_complete <- cbind(ts1[,excludeCols,with=FALSE], complete(imp_merged))
# write_csv(ts1_complete, "./data_trans/ts1_mice_v7.csv")
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v7.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v7.rda")

ts2_dum <- ts2_dum[order(ts2_dum$filter, ts2_dum$ID),]
write_csv(ts2_dum, "./data_trans/ts2Trans_v7.csv")
