...
# Count NAs by row
ts1$cntNA <- rowSums(is.na(ts1[, !colnames(ts1) %in% c("ID","target","filter"), with=FALSE]))
ts1$cntZero <- rowSums(ts1[, !colnames(ts1) %in% c("ID","target","filter"), with=FALSE] == 0, na.rm=TRUE)
...
highCardFacts <- colnames(ts1_pp[,factorCols,with=FALSE])[sapply(ts1_pp[,factorCols,with=FALSE], function(x) length(unique(x))>100)]
...
dummy <- dummyVars( ~. -1, data = ts1_pp[,1:ncol(ts1_pp),with=FALSE])
ts1_dum <- data.frame(predict(dummy, ts1_pp[,1:ncol(ts1_pp),with=FALSE]))
...
write_csv(ts1_dum, "./data_trans/ts1Trans_v2.csv")
