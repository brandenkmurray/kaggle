...
# }
# 
# save(imp_merged, file="./data_trans/mice_v9.rda")
# ts1_complete <- cbind(ts1[,excludeCols,with=FALSE], complete(imp_merged))
# write_csv(ts1_complete, "./data_trans/ts1_mice_v9.csv")
...
for(i in 1:length(pairs)) {
  ...
  ts2[,name] <- cat2WayAvg(data = ts2, var1 = pairs[[i]][1], var2 = pairs[[i]][2], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 40, f = 20, r_k = 0.04)
}
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v9.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v9.rda")

ts2_dum <- ts2_dum[order(ts2_dum$filter, ts2_dum$ID),]
write_csv(ts2_dum, "./data_trans/ts2Trans_v9.csv")
