...
out <- foreach(i=1:length(pairs), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, pairs[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 15, f = 7, r_k = 0.03, cv=cvFoldsList)
...
out <- foreach(i=1:length(triplets), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, triplets[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 15, f = 7, r_k = 0.03, cv=cvFoldsList)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
out <- foreach(i=1:length(quads), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, quads[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 15, f = 7, r_k = 0.03, cv=cvFoldsList)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
#                  tmp <- cat5WayAvgCV(data = ts2, var1 = quints[[i]][1], var2 = quints[[i]][2], var3 = quints[[i]][3], var4 = quints[[i]][4], var5 = quints[[i]][5], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 15, f = 7, r_k = 0.03, cv=cvFoldsList10)
#                  tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
#                  list(tmp, name)
#                }
# stopCluster(cl)
...
#                  tmp <- cat6WayAvgCV(data = ts2, var1 = sextups[[i]][1], var2 = sextups[[i]][2], var3 = sextups[[i]][3], var4 = sextups[[i]][4], var5 = sextups[[i]][5], var6 = sextups[[i]][6], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 15, f = 7, r_k = 0.03, cv=cvFoldsList10)
#                  tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
#                  list(tmp, name)
#                }
# stopCluster(cl)
...
out <- foreach(i=1:length(septups), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, septups[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 15, f = 7, r_k = 0.03, cv=cvFoldsList)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
featCor <- corFast(ts2[filter==0,numCols,with=FALSE], nThreads=threads)
hc <- findCorrelation(featCor, cutoff=0.997 ,names=TRUE)  
hc <- sort(hc)
save(featCor, file="./data_trans/featCor_v33.rda")
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v33.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v33.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v33.csv")
