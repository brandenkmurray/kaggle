...
miceImp <- data.table(read_csv("./data_trans/ts1_mice_v5.csv"))
...
km_y_summ4 <- data.table(ID=ts1$ID, cluster=as.factor(make.names(ts1_kmeans4$cluster)), filter=ts1$filter)
ts1_km4Cast <- dcast.data.table(km_y_summ4, ID ~ cluster, fun.aggregate = length, value.var="cluster")
colnames(ts1_km4Cast)[2:5] <- paste0(colnames(ts1_km4Cast)[2:5], "_km4")
ts1 <- merge(ts1, ts1_km4Cast, by="ID", sort=FALSE)
...
km_y_summ5 <- data.table(ID=ts1$ID, cluster=as.factor(make.names(ts1_kmeans5$cluster)), filter=ts1$filter)
ts1_km5Cast <- dcast.data.table(km_y_summ5, ID ~ cluster, fun.aggregate = length, value.var="cluster")
colnames(ts1_km5Cast)[2:6] <- paste0(colnames(ts1_km5Cast)[2:6], "_km5")
ts1 <- merge(ts1, ts1_km5Cast, by="ID", sort=FALSE)
...
km_y_summ6 <- data.table(ID=ts1$ID, cluster=as.factor(make.names(ts1_kmeans6$cluster)), filter=ts1$filter)
ts1_km6Cast <- dcast.data.table(km_y_summ6, ID ~ cluster, fun.aggregate = length, value.var="cluster")
colnames(ts1_km6Cast)[2:7] <- paste0(colnames(ts1_km6Cast)[2:7], "_km6")
ts1 <- merge(ts1, ts1_km6Cast, by="ID", sort=FALSE)
...
km_y_summ7 <- data.table(ID=ts1$ID, cluster=as.factor(make.names(ts1_kmeans7$cluster)), filter=ts1$filter)
ts1_km7Cast <- dcast.data.table(km_y_summ7, ID ~ cluster, fun.aggregate = length, value.var="cluster")
colnames(ts1_km7Cast)[2:8] <- paste0(colnames(ts1_km7Cast)[2:8], "_km7")
ts1 <- merge(ts1, ts1_km7Cast, by="ID", sort=FALSE, suffixes = "_km7")
...
# Amscombe transform
ams <- 2*sqrt(miceImp[,numCols,with=FALSE]+(3/8))
ts1[,numCols] <- ams
...
# }
# 
# save(imp_merged, file="./data_trans/mice_v12.rda")
# ts1_complete <- cbind(ts1[,excludeCols,with=FALSE], complete(imp_merged))
# write_csv(ts1_complete, "./data_trans/ts1_mice_v12.csv")
...
for(i in 1:length(pairs)) {
  ...
  ts2[,name] <- cat2WayAvg(data = ts2, var1 = pairs[[i]][1], var2 = pairs[[i]][2], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 40, f = 30, r_k = 0.05)
}
...
for (i in 1:length(triplets)){
  ...
  if (sd(tmp[ts2$filter==0]) == 0) next else # exclude columns with no variance in the training set
    ts2[,name] <- tmp
}
...
numCols <- colnames(ts2_pp[,-excludeCols, with=FALSE])[sapply(ts2_pp[,-excludeCols, with=FALSE], is.numeric)]
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v12.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v12.rda")

ts2_dum <- ts2_dum[order(ts2_dum$filter, ts2_dum$ID),]
write_csv(ts2_dum, "./data_trans/ts2Trans_v12.csv")
