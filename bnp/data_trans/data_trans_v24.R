...
out <- foreach(i=1:length(pairs), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, pairs[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 50, f = 30, r_k = 0.07, cv=cvFoldsList10)
...
out <- foreach(i=1:length(triplets), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, triplets[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 50, f = 30, r_k = 0.07, cv=cvFoldsList10)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
#                  tmp <- cat4WayAvgCV(data = ts2, var1 = quads[[i]][1], var2 = quads[[i]][2], var3 = quads[[i]][3], var4 = quads[[i]][4], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 10, r_k = 0.03, cv=cvFoldsList10)
#                  tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
#                  list(tmp, name)
#                }
# stopCluster(cl)
...
out <- foreach(i=1:length(septups), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, septups[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 50, f = 30, r_k = 0.07, cv=cvFoldsList10)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
## Add 9-way counts
################
# nonuplets <- combn(c("v3","v22","v24","v47","v52","v56", "v66","v72", "v74", "v79","v110","v113","v125","v129"), 9, simplify=FALSE)
# set.seed(903)
...
#                  tmp <- catNWayAvgCV(data = ts2, nonuplets[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 10, r_k = 0.03, cv=cvFoldsList10)
#                  tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
#                  list(tmp, name)
#                }
# stopCluster(cl)
...
for(ii in highCardFacts) {
  ...
  ts2[,name] <- cat2WayAvgCV(data = ts2, var1 = ii, var2 = "dummy", y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 50, f = 30, r_k = 0.07, cv = cvFoldsList10)
}
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v24.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v24.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v24.csv")
