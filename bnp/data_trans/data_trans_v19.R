...
################
#Remove v52, add v110
septups <- combn(c("v22","v24","v47","v56", "v66","v72", "v74", "v79","v110","v113","v125","v129"), 7, simplify=FALSE)
# set.seed(903)
...
for(ii in highCardFacts) {
  ...
  ts2[,name] <- cat2WayAvgCV(data = ts2, var1 = ii, var2 = "dummy", y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 10, r_k = 0.05, cv = cvFoldsList)
}
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v19.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v19.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v19.csv")
