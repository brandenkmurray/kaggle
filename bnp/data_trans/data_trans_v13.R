...
# }
# 
# save(imp_merged, file="./data_trans/mice_v13.rda")
...
for(i in 1:length(pairs)) {
  ...
  ts2[,name] <- cat2WayAvgCV(data = ts2, var1 = pairs[[i]][1], var2 = pairs[[i]][2], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 40, f = 30, r_k = 0.05, cv=cvFoldsList)
}
...
for(i in 1:length(triplets)) {
  ...
  ts2[,name] <- cat3WayAvgCV(data = ts2, var1 = triplets[[i]][1], var2 = triplets[[i]][2], var3 = triplets[[i]][3], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 60, f = 30, r_k = 0.05, cv=cvFoldsList)
}
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v13.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v13.rda")

ts2_dum <- ts2_dum[order(ts2_dum$filter, ts2_dum$ID),]
write_csv(ts2_dum, "./data_trans/ts2Trans_v13.csv")
