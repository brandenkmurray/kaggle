...
# ts1[is.na(ts1)] <- -1
amelImpute <- amelia(ts1, m=3, idvars=1:2)

tme <- Sys.time()
ts1_impute <- knnImputation(ts1[filter==0, -excludeCols, with=FALSE], k=10, scale=T)
Sys.time() - tme

pp <- preProcess(ts1[filter==0, -excludeCols, with=FALSE], method=c("knnImpute"), k=10)
ts1 <- predict(pp, ts1)
