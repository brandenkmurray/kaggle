...
out <- foreach(i=1:length(pairs), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, pairs[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 8.5, r_k = 0.04, cv=cvFoldsList)
...
out <- foreach(i=1:length(triplets), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, triplets[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 8.5, r_k = 0.04, cv=cvFoldsList)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
out <- foreach(i=1:length(quads), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, quads[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 8.5, r_k = 0.04, cv=cvFoldsList)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
out <- foreach(i=1:length(septups), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table","VGAM")) %dorng% {
                 ...
                 tmp <- catNWayAvgCV(data = ts2, septups[[i]], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 8.5, r_k = 0.04, cv=cvFoldsList)
                 tmp <- logit(pmin(pmax(tmp, 1e-15), 1-1e-15))
                 list(tmp, name)
               }
stopCluster(cl)
...
hc <- sort(hc)
save(featCor, file="./data_trans/featCor_v28.rda")
...
for(ii in highCardFacts) {
  ...
  ts2[,name] <- catNWayAvgCV(data = ts2, c(ii, "dummy"), y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 20, f = 8.5, r_k = 0.04, cv=cvFoldsList)
}
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v28.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v28.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v28.csv")
