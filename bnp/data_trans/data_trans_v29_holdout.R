...
## Dmitry's holdout
#################
holdout <- read.csv("./Dmitry/holdout.csv")
holdout <- holdout[order(holdout$ID),]
...
ts1 <- cbind(pred0=mean(t1$target), dummy="A", filter=c(holdout$holdout,rep(2, nrow(s1))), ts1)
ts1 <- ts1[order(ts1$filter, ts1$ID), ]

# Create Folds
set.seed(2015)
cvFoldsList <- createFolds(ts1$target[ts1$filter==0],k=5, list=TRUE)
...
hc <- sort(hc)
save(featCor, file="./data_trans/featCor_v29_holdout.rda")
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v29_holdout.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v29_holdout.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v29_holdout.csv")
