...
feat_gold <- gold_features(featCorDF, 300)
...
if (length(c(hc,as.character(featCorDF$V2[1:300])))>0)
  ts2 <- ts2[,-c(hc,as.character(featCorDF$V2[1:300])),with=FALSE]
######################################################
...
set.seed(201601)
tsne_feats <- Rtsne(data.matrix(ts2_pca_feats), dims=2, initial_dims = 100, perplexity=80, theta=0.1, pca=FALSE, check_duplicates=FALSE, max_iter=500, verbose=TRUE)
tsne_Y <- as.data.frame(tsne_feats$Y)
colnames(tsne_Y) <- c("tsne_1", "tsne_2")
write.csv(tsne_Y, "./data_trans/tsne_v21.csv", row.names=FALSE)
...
ts2<- cbind(ts2, tsne_Y[,1:2])

###################
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v21.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v21.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v21.csv")
