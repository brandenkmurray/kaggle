...
stopCluster(cl)
# Combine results
ts2 <- cbind(ts2, pairCnts, pairMeans, tripCnts, tripMeans, quadCnts, quadMeans)
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v15.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v15.rda")

ts2_dum <- ts2_dum[order(ts2_dum$filter, ts2_dum$ID),]
write_csv(ts2_dum, "./data_trans/ts2Trans_v15.csv")
