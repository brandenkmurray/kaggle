...
# v111 and v117 are the same -- just different labels
ts1[,v117:=NULL]
...
# }
# 
# save(imp_merged, file="./data_trans/mice_v11.rda")
# ts1_complete <- cbind(ts1[,excludeCols,with=FALSE], complete(imp_merged))
# write_csv(ts1_complete, "./data_trans/ts1_mice_v11.csv")
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v11.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v11.rda")

ts2_dum <- ts2_dum[order(ts2_dum$filter, ts2_dum$ID),]

set.seed(201601)
tsne_feats <- Rtsne(data.matrix(ts2_dum[,4:ncol(ts2_dum)]), dims=2, initial_dims = 200, perplexity=30, theta=0.1, check_duplicates=TRUE, max_iter=500, verbose=TRUE)
tsne_Y <- as.data.frame(tsne_feats$Y)
colnames(tsne_Y) <- c("tsne_1", "tsne_2")
write.csv(tsne_Y, "./data_trans/tsne_v11.csv", row.names=FALSE)
tsne_Y$target <- as.factor(make.names(ts2_dum$target))
tsne_Y$filter <- ts2_dum$filter
(gg <- ggplot(tsne_Y[tsne_Y$filter==0,], aes(x=tsne_1, y=tsne_2, colour=target)) + geom_point(size=1))

# ts1Trans <- cbind(ts1Trans, tsne_Y[,1:2])

write_csv(ts2_dum, "./data_trans/ts2Trans_v11.csv")
