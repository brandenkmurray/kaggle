...
## Add 3-way counts
################
triplets <- combn(c("v22","v24","v47", "v52", "v56", "v66","v72", "v79", "v110","v113","v125","v129"), 3, simplify=FALSE)
set.seed(123)
...
## Add 4-way counts
################
quads <- combn(c("v22","v24","v47","v56", "v66","v72", "v79","v113","v125","v129"), 4, simplify=FALSE)
set.seed(123)
...
out <- foreach(i=1:length(quads), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table")) %dorng% {
                 ...
                 tmp <- cat4WayAvgCV(data = ts2, var1 = quads[[i]][1], var2 = quads[[i]][2], var3 = quads[[i]][3], var4 = quads[[i]][4], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 30, f = 20, r_k = 0.04, cv=cvFoldsList)
                 list(tmp, name)
               }
...
## Add 5-way counts
################
quints <- combn(c("v22","v24","v47","v56", "v66","v72", "v79","v113","v125","v129"), 5, simplify=FALSE)
set.seed(123)
...
out <- foreach(i=1:length(quints), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table")) %dorng% {
                 ...
                 tmp <- cat5WayAvgCV(data = ts2, var1 = quints[[i]][1], var2 = quints[[i]][2], var3 = quints[[i]][3], var4 = quints[[i]][4], var5 = quints[[i]][5], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 30, f = 20, r_k = 0.04, cv=cvFoldsList)
                 list(tmp, name)
               }
...
## Add 6-way counts
################
sextups <- combn(c("v22","v24","v47","v56", "v66","v72", "v79","v113","v125","v129"), 6, simplify=FALSE)
set.seed(123)
...
out <- foreach(i=1:length(sextups), .combine='comb', .multicombine=TRUE,
               .init=list(list(), list()), .packages=c("sqldf", "data.table")) %dorng% {
                 ...
                 tmp <- cat6WayAvgCV(data = ts2, var1 = sextups[[i]][1], var2 = sextups[[i]][2], var3 = sextups[[i]][3], var4 = sextups[[i]][4], var5 = sextups[[i]][5], var6 = sextups[[i]][6], y = "target",pred0 = "pred0",filter = ts2$filter==0, k = 30, f = 20, r_k = 0.04, cv=cvFoldsList)
                 list(tmp, name)
               }
...
## Write CSV file
###################
write.csv(as.data.frame(helpCols), "./data_trans/helpCols_v16.csv", row.names=FALSE)
save(helpCols, file="./data_trans/helpCols_v16.rda")

ts2 <- ts2[order(ts2$filter, ts2$ID),]
write_csv(ts2, "./data_trans/ts2Trans_v16.csv")
