...
# kknn3_stack_preds <- matrix(0, nrow=nrow(t1kknn), ncol=1)
...
#                 distance=kknn_dist,
#                 kernel=kknn_kernel )
#   kknn3_stack_preds[cvFoldsList[[1]],] <- kknn3$prob[,2]
#   logLossTable[i,2] <- LogLoss(as.numeric(t1kknn$target[cvFoldsList[[1]]])-1, kknn3$prob[,2])
# }
...
kknn3_stack_preds <- data.frame(index=unlist(cvFoldsList), cvPred=out)
kknn3_stack_preds <- kknn3_stack_preds[order(kknn3_stack_preds$index),]
LogLoss(as.numeric(t1kknn$target)-1, kknn3_stack_preds$cvPred)


cvPreds <- kknn3_stack_preds$cvPred
...
write.csv(cvPreds, "./stack_models/cvPreds/cvPreds_kknn3.csv", row.names=FALSE)

# LogLoss(model.matrix(~class-1, t1kknn[-cvFoldsList[[5]],"class", with=FALSE]), kknn3_stack_preds$prob)
# LogLoss(model.matrix(~class-1, t1kknn[,"class", with=FALSE]), kknn3_stack_preds)


kknn3full <- kknn(target ~ .,
              train=t1kknn, 
              test=s1kknn, 
              k=kknn_k, 
              distance=kknn_dist,
...
testPreds <- kknn3full$prob[,2]
names(testPreds) <- "PredictedProb"
testPreds <- cbind(ts1Trans[filter==2,"ID",with=FALSE], testPreds)
write.csv(testPreds, "./stack_models/testPreds/testPreds_kknn3.csv", row.names=FALSE)
