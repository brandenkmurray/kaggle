...
# kknn2_stack_preds <- matrix(0, nrow=nrow(t1kknn), ncol=1)
...
#                 distance=kknn_dist,
#                 kernel=kknn_kernel )
#   kknn2_stack_preds[cvFoldsList[[1]],] <- kknn2$prob[,2]
#   logLossTable[i,2] <- LogLoss(as.numeric(t1kknn$target[cvFoldsList[[1]]])-1, kknn2$prob[,2])
# }
...
kknn2_stack_preds <- data.frame(index=unlist(cvFoldsList), cvPred=out)
kknn2_stack_preds <- kknn2_stack_preds[order(kknn2_stack_preds$index),]
LogLoss(as.numeric(t1kknn$target)-1, kknn2_stack_preds$cvPred)


cvPreds <- kknn2_stack_preds$cvPred
samp <- read.csv('sample_submission.csv')
cnames <- paste("kknn2", names(samp)[2:ncol(samp)], sep="_")
names(cvPreds) <- cnames
cvPreds <- cbind(ts1Trans[filter==0,"ID",with=FALSE], cvPreds)
write.csv(cvPreds, "./stack_models/cvPreds/cvPreds_kknn2.csv", row.names=FALSE)

# LogLoss(model.matrix(~class-1, t1kknn[-cvFoldsList[[5]],"class", with=FALSE]), kknn2_stack_preds$prob)
# LogLoss(model.matrix(~class-1, t1kknn[,"class", with=FALSE]), kknn2_stack_preds)


kknn2full <- kknn(target ~ .,
                  train=t1kknn, 
                  test=s1kknn, 
                  k=kknn_k, 
                  distance=kknn_dist,
                  kernel=kknn_kernel)
save(kknn2full, file="./stack_models/kknn2full.rda")

testPreds <- kknn2full$prob[,2]
names(testPreds) <- "PredictedProb"
testPreds <- cbind(ts1Trans[filter==2,"ID",with=FALSE], testPreds)
write.csv(testPreds, "./stack_models/testPreds/testPreds_kknn2.csv", row.names=FALSE)
