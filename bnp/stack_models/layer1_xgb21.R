...
# write.csv(tsne_Y, "./stack_models/tsne_xgb21.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb21cv, file="./stack_models/xgb21cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb21cv$pred), "./stack_models/cvPreds/cvPreds_xgb21.csv", row.names=FALSE)

minLossRound <- which.min(xgb21cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb21full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb21full, file="./stack_models/xgb21full.rda")

preds <- predict(xgb21full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb21.csv", row.names=FALSE)


xgb21Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb21full)
write.csv(xgb21Imp, "./stack_models/xgb21Imp.csv", row.names=FALSE)
