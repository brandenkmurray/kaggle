...
# xgbImpVars <- data.table(read_csv("./stack_models/xgb16Imp.csv"))
...
grid <- expand.grid(eta=c(0.003,0.007,0.01,0.013,0.017,0.02,0.025,0.03,0.04,0.05), md=seq(3,17,1), mcw=c(rep(1,20),seq(2,20,2)), ss=seq(.25,1,.05), csbt=seq(.25,1,.05))
set.seed(200)
models <- 50
...
for (i in 1:models) {
  ...
  xgb16cv <- xgb.cv(data=dtrain,
                    ...
                    maximize=FALSE)
  Sys.time() - tme
  bagPreds[,i] <- xgb16cv$pred
  min(xgb16cv$dt$test.logloss.mean)
  stopRounds[i,"round"] <- which.min(xgb16cv$dt$test.logloss.mean)
  stopRounds[i,"minLoss"] <- min(xgb16cv$dt$test.logloss.mean)
  write_csv(stopRounds, "./stack_models/stopRounds_xgb16cv.csv")
  
  #Move the CSV to inside the loop in case there's a crash midway through - Can continue loop from where it left off
  cvPreds <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==0], xgb16cv_pred=bagPreds)
  colnames(cvPreds) <- c("ID", paste0("cvPreds_xgb16_mod",1:models))
  write.csv(cvPreds, "./stack_models/cvPreds/cvPreds_xgb16.csv", row.names=FALSE) 
 
  set.seed(2015+i)
  xgb16 <- xgb.train(data=dtrain,
                    params=param,
                    nrounds=round(stopRounds[i,2]*1.125))
  bagPreds_test[,i] <- predict(xgb16, newdata=data.matrix(ts1Trans[filter==2, c(varnames),with=FALSE]))
  
  submission <- data.frame(ID=sub$ID, PredictedProb=bagPreds_test)
  colnames(submission) <- c("ID", paste0("testPreds_xgb16_mod",1:models))
  write.csv(submission, "./stack_models/testPreds/testPreds_xgb16.csv", row.names=FALSE)
}
