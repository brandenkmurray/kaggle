...
# write.csv(tsne_Y, "./stack_models/tsne_xgb17.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb17cv, file="./stack_models/xgb17cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb17cv$pred), "./stack_models/cvPreds/cvPreds_xgb17.csv", row.names=FALSE)

minLossRound <- which.min(xgb17cv$dt$test.logloss.mean)
...
preds <- predict(xgb17full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb17.csv", row.names=FALSE)


xgb17Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb17full)
write.csv(xgb17Imp, "./stack_models/xgb17Imp.csv", row.names=FALSE)
