...
Sys.time() - tme
save(xgb42cv, file="./stack_models/xgb42cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb42cv$pred), "./stack_models/cvPreds/cvPreds_xgb42.csv", row.names=FALSE)

minLossRound <- which.min(xgb42cv$dt$test.logloss.mean)
...
xgb42full <- xgb.train(data = dtrain,
                      ...
                      print.every.n = 20)
Sys.time() - tme
save(xgb42full, file="./stack_models/xgb42full.rda")
...
write.csv(submission, "./stack_models/testPreds/testPreds_xgb42.csv", row.names=FALSE)


xgb42Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb42full)
write.csv(xgb42Imp, "./stack_models/xgb42Imp.csv", row.names=FALSE)
