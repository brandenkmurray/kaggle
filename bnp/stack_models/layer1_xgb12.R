...
ts1Trans <- data.table(read.csv("./data_trans/ts2Trans_v9.csv"))
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb12.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb12cv, file="./stack_models/xgb12cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb12cv$pred), "./stack_models/cvPreds/cvPreds_xgb12.csv", row.names=FALSE)

minLossRound <- which.min(xgb12cv$dt$test.logloss.mean)
...
preds <- predict(xgb12full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb12.csv", row.names=FALSE)


xgb12Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==2, c(varnames), with=FALSE]), model=xgb12full)
write.csv(xgb12Imp, "./stack_models/xgb12Imp.csv", row.names=FALSE)
