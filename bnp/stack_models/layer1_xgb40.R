...
preds <- predict(xgb40full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
...
xgb40Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb40full)
