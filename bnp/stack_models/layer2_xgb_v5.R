...
cvPreds <- data.frame(ID=t1$ID, PredictProb=xgbLay2_v5$pred)
write_csv(cvPreds, "./stack_models/layer2Preds/cvPreds_xgbLay2_v5.csv")
rounds <- floor(which.min(xgbLay2_v5$dt$test.logloss.mean) * 1.08)
...
write.csv(submission, "./stack_models/layer2Preds/xgbLay2_v5preds.csv", row.names=FALSE)

xgbLay2_v5_Imp <- xgb.importance(feature_names = colnames(lay1preds[,2:ncol(lay1preds)]), model=xgbLay2_v5_full)
write_csv(xgbLay2_v5_Imp, "./stack_models/xgbLay2_v5_Imp.csv")
