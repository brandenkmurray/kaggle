...
preds <- predict(xgb38full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
...
xgb38Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb38full)
