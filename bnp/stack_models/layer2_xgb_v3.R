...
set.seed(201601)
(tme <- Sys.time())
xgbLay2_xgb_v3 <- xgb.cv(data = dtrain,
                         params = param,
                         nrounds = 50000,
                         maximize=FALSE,
                         prediction=TRUE,
...
Sys.time() - tme
save(xgbLay2_xgb_v3, file="./stack_models/xgbLay2_xgb_v3.rda")


rounds <- floor(which.min(xgbLay2_xgb_v3$dt$test.logloss.mean) * 1.08)
...
write.csv(submission, "./stack_models/layer2Preds/xgbLay2_v3preds.csv", row.names=FALSE)

xgbLay2_v3_Imp <- xgb.importance(feature_names = colnames(lay1fullpreds), model=xgbLay2_v3_full)
write_csv(xgbLay2_v3_Imp, "./stack_models/xgbLay2_v3_Imp.csv")
