...
# write.csv(tsne_Y, "./stack_models/tsne_xgb28.csv", row.names=FALSE)
...
for (i in 1:length(cvFoldsList)) {
  ...
  foldPreds[cvFoldsList[[i]]] <- predict(xgb28, data.matrix(ts1Trans[filter==0][cvFoldsList[[i]], c(varnames),with=FALSE]))
  testPreds <- testPreds + predict(xgb28, data.matrix(ts1Trans[filter==2, c(varnames),with=FALSE]))
...
write_csv(data.frame(ID=ts1Trans$ID[ts1Trans$filter==0], PredictedProb=foldPreds), "./stack_models/cvPreds/cvPreds_xgb28.csv")

submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=testPreds_adj)
write_csv(submission, "./stack_models/testPreds/testPreds_xgb28.csv")
