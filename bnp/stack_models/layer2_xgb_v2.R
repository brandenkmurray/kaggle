...
set.seed(201601)
(tme <- Sys.time())
xgbLay2_xgb_v2 <- xgb.cv(data = dtrain,
                         params = param,
                         nrounds = 50000,
                         maximize=FALSE,
                         prediction=TRUE,
...
Sys.time() - tme
save(xgbLay2_xgb_v2, file="./stack_models/xgbLay2_xgb_v2.rda")


rounds <- floor(which.min(xgbLay2_xgb_v2$dt$test.logloss.mean) * 1.08)
...
set.seed(201601)
(tme <- Sys.time())
xgbLay2_v2_full <- xgb.train(data = dtrain,
                        params = param,
                        nrounds = rounds,
                        maximize=FALSE,
                        # watchlist=watchlist,
                        print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v2_full, file="./stack_models/xgbLay2_v2_full.rda")

lay2preds <- predict(xgbLay2_v2_full, newdata=data.matrix(lay1fullpreds))
samp <- read_csv('sample_submission.csv')
submission <- data.frame(ID=sort(samp$ID), PredictedProb=lay2preds)
write.csv(submission, "./stack_models/layer2Preds/xgbLay2_v2preds.csv", row.names=FALSE)

xgbLay2_v2_Imp <- xgb.importance(feature_names = colnames(lay1fullpreds), model=xgbLay2_v2_full)
write_csv(xgbLay2_v2_Imp, "./stack_models/xgbLay2_v2_Imp.csv")
