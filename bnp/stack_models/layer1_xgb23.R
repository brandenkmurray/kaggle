...
setwd("~/bnp")
ts1Trans <- fread("./data_trans/ts2Trans_v16.csv")
...
grid <- expand.grid(eta=c(0.003,0.01,0.017,0.025,0.05), md=seq(3,13,1), mcw=c(rep(1,20),seq(2,20,2),30,40,50), ss=seq(.25,1,.05), csbt=seq(.25,1,.05))
set.seed(242)
models <- 25
...
for (i in 1:models) {
  ...
  xgb23cv <- xgb.cv(data=dtrain,
                    ...
                    maximize=FALSE)
  Sys.time() - tme
  bagPreds[,i] <- xgb23cv$pred
  min(xgb23cv$dt$test.logloss.mean)
  stopRounds[i,"round"] <- which.min(xgb23cv$dt$test.logloss.mean)
  stopRounds[i,"minLoss"] <- min(xgb23cv$dt$test.logloss.mean)
  write_csv(stopRounds, "./stack_models/stopRounds_xgb23cv.csv")
  
  #Move the CSV to inside the loop in case there's a crash midway through - Can continue loop from where it left off
  cvPreds <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==0], xgb23cv_pred=bagPreds)
  colnames(cvPreds) <- c("ID", paste0("cvPreds_xgb23_mod",1:models))
  write.csv(cvPreds, "./stack_models/cvPreds/cvPreds_xgb23.csv", row.names=FALSE) 
  
  set.seed(2015+i)
  xgb23 <- xgb.train(data=dtrain,
                     params=param,
                     nrounds=round(stopRounds[i,2]*1.125))
  bagPreds_test[,i] <- predict(xgb23, newdata=data.matrix(ts1Trans[filter==2, c(varnames),with=FALSE]))
  
  submission <- data.frame(ID=sub$ID, PredictedProb=bagPreds_test)
  colnames(submission) <- c("ID", paste0("testPreds_xgb23_mod",1:models))
  write.csv(submission, "./stack_models/testPreds/testPreds_xgb23.csv", row.names=FALSE)
