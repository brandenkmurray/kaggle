...
ts1Trans <- data.table(read.csv("./data_trans/ts2Trans_v15.csv"))
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb20.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb20cv, file="./stack_models/xgb20cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb20cv$pred), "./stack_models/cvPreds/cvPreds_xgb20.csv", row.names=FALSE)

minLossRound <- which.min(xgb20cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb20full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb20full, file="./stack_models/xgb20full.rda")

preds <- predict(xgb20full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb20.csv", row.names=FALSE)


xgb20Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb20full)
write.csv(xgb20Imp, "./stack_models/xgb20Imp.csv", row.names=FALSE)
