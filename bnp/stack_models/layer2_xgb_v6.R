...
cvPreds <- data.frame(ID=t1$ID, PredictProb=xgbLay2_v6$pred)
write_csv(cvPreds, "./stack_models/layer2Preds/cvPreds_xgbLay2_v6.csv")
rounds <- floor(which.min(xgbLay2_v6$dt$test.logloss.mean) * 1.08)
...
write.csv(submission, "./stack_models/layer2Preds/xgbLay2_v6preds.csv", row.names=FALSE)

xgbLay2_v6_Imp <- xgb.importance(feature_names = colnames(lay1preds[,2:ncol(lay1preds)]), model=xgbLay2_v6_full)
write_csv(xgbLay2_v6_Imp, "./stack_models/xgbLay2_v6_Imp.csv")
