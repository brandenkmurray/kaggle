...
ts1Trans <-fread("./data_trans/ts2Trans_v21.csv")
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb32.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb32cv, file="./stack_models/xgb32cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb32cv$pred), "./stack_models/cvPreds/cvPreds_xgb32.csv", row.names=FALSE)

minLossRound <- which.min(xgb32cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb32full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb32full, file="./stack_models/xgb32full.rda")

preds <- predict(xgb32full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb32.csv", row.names=FALSE)


xgb32Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb32full)
write.csv(xgb32Imp, "./stack_models/xgb32Imp.csv", row.names=FALSE)
