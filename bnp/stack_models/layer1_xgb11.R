...
ts1Trans <- data.table(read.csv("./data_trans/ts2Trans_v8.csv"))
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb11.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb11cv, file="./stack_models/xgb11cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb11cv$pred), "./stack_models/cvPreds/cvPreds_xgb11.csv", row.names=FALSE)

minLossRound <- which.min(xgb11cv$dt$test.logloss.mean)
...
preds <- predict(xgb11full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb11.csv", row.names=FALSE)


xgb11Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==2, c(varnames), with=FALSE]), model=xgb11full)
write.csv(xgb11Imp, "./stack_models/xgb11Imp.csv", row.names=FALSE)
