...
ts1Trans <-fread("./data_trans/ts2Trans_v19.csv")
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb25.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb25cv, file="./stack_models/xgb25cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb25cv$pred), "./stack_models/cvPreds/cvPreds_xgb25.csv", row.names=FALSE)

minLossRound <- which.min(xgb25cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb25full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb25full, file="./stack_models/xgb25full.rda")

preds <- predict(xgb25full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb25.csv", row.names=FALSE)


xgb25Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb25full)
write.csv(xgb25Imp, "./stack_models/xgb25Imp.csv", row.names=FALSE)
