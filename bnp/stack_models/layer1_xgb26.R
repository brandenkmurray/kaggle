...
# write.csv(tsne_Y, "./stack_models/tsne_xgb26.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb26cv, file="./stack_models/xgb26cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb26cv$pred), "./stack_models/cvPreds/cvPreds_xgb26.csv", row.names=FALSE)

minLossRound <- which.min(xgb26cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb26full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb26full, file="./stack_models/xgb26full.rda")

preds <- predict(xgb26full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb26.csv", row.names=FALSE)


xgb26Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb26full)
write.csv(xgb26Imp, "./stack_models/xgb26Imp.csv", row.names=FALSE)
