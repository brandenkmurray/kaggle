...
# kknn4_stack_preds <- matrix(0, nrow=nrow(t1kknn), ncol=1)
...
#                 distance=kknn_dist,
#                 kernel=kknn_kernel )
#   kknn4_stack_preds[cvFoldsList[[1]],] <- kknn4$prob[,2]
#   logLossTable[i,2] <- LogLoss(as.numeric(t1kknn$target[cvFoldsList[[1]]])-1, kknn4$prob[,2])
# }
...
kknn4_stack_preds <- data.frame(index=unlist(cvFoldsList), cvPred=out)
kknn4_stack_preds <- kknn4_stack_preds[order(kknn4_stack_preds$index),]
LogLoss(as.numeric(t1kknn$target)-1, kknn4_stack_preds$cvPred)


cvPreds <- kknn4_stack_preds$cvPred
...
write.csv(cvPreds, "./stack_models/cvPreds/cvPreds_kknn4.csv", row.names=FALSE)

# LogLoss(model.matrix(~class-1, t1kknn[-cvFoldsList[[5]],"class", with=FALSE]), kknn4_stack_preds$prob)
# LogLoss(model.matrix(~class-1, t1kknn[,"class", with=FALSE]), kknn4_stack_preds)


kknn4full <- kknn(target ~ .,
                  train=t1kknn, 
                  test=s1kknn, 
                  k=kknn_k, 
                  distance=kknn_dist,
...
testPreds <- kknn4full$prob[,2]
names(testPreds) <- "PredictedProb"
testPreds <- cbind(ts1Trans[filter==2,"ID",with=FALSE], testPreds)
write.csv(testPreds, "./stack_models/testPreds/testPreds_kknn4full.csv", row.names=FALSE)
