...
setwd("~/bnp")
ts1Trans <- data.table(read.csv("./data_trans/ts2Trans_v6.csv"))
# xgbImpVars <- data.table(read_csv("./stack_models/xgb9Imp.csv"))
...
grid <- expand.grid(eta=c(0.003,0.007,0.01,0.013,0.017,0.02,0.025,0.03,0.04,0.05), md=seq(1,17,1), mcw=c(rep(1,30),seq(2,20,2)), ss=seq(.25,1,.05), csbt=seq(.25,1,.05))
set.seed(69)
models <- 30
...
for (i in 1:nrow(grid_samp)) {
  ...
  xgb9cv <- xgb.cv(data=dtrain,
                    ...
                    maximize=FALSE)
  Sys.time() - tme
  bagPreds[,i] <- xgb9cv$pred
  min(xgb9cv$dt$test.logloss.mean)
  stopRounds[i,"round"] <- which.min(xgb9cv$dt$test.logloss.mean)
  write_csv(stopRounds, "./stack_models/stopRounds_xgb9cv.csv")
  
  #Move the CSV to inside the loop in case there's a crash midway through - Can continue loop from where it left off
  cvPreds <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==0], xgb9cv_pred=bagPreds)
  colnames(cvPreds) <- c("ID", paste0("cvPreds_xgb9_mod",1:models))
  write.csv(cvPreds, "./stack_models/cvPreds/cvPreds_xgb9.csv", row.names=FALSE) 
 
  set.seed(2015+i)
  xgb9 <- xgb.train(data=dtrain,
                    params=param,
                    nrounds=round(stopRounds[i,2]*1.125))
  bagPreds_test[,i] <- predict(xgb9, newdata=data.matrix(ts1Trans[filter==2, c(varnames),with=FALSE]))
  
  submission <- data.frame(ID=sub$ID, PredictedProb=bagPreds_test)
  colnames(submission) <- c("ID", paste0("testPreds_xgb9_mod",1:models))
  write.csv(submission, "./stack_models/testPreds/testPreds_xgb9.csv", row.names=FALSE)
}


submission2 <- data.frame(ID=sub$ID, PredictedProb=rowMeans(bagPreds_test))
write.csv(submission2, "./stack_models/testPreds/testPreds_means/testPreds_xgb9_means.csv", row.names=FALSE)
