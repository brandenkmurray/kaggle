...
ts1Trans <- data.table(read.csv("./data_trans/ts2Trans_v18.csv"))
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb24.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb24cv, file="./stack_models/xgb24cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb24cv$pred), "./stack_models/cvPreds/cvPreds_xgb24.csv", row.names=FALSE)

minLossRound <- which.min(xgb24cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb24full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb24full, file="./stack_models/xgb24full.rda")

preds <- predict(xgb24full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb24.csv", row.names=FALSE)


xgb24Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb24full)
write.csv(xgb24Imp, "./stack_models/xgb24Imp.csv", row.names=FALSE)
