...
grid <- expand.grid(eta=c(0.003,0.01,0.025,.05), md=c(3,5,7,13,17), mcw=c(1,5,10,20,50,100), ss=c(0.25,0.5,0.75,1), csbt=c(0.25,0.50,0.75,1))
set.seed(44)
models <- 100
...
for (i in 1:nrow(grid_samp)) {
  ...
  xgb5cv <- xgb.cv(data=dtrain,
                    ...
                    maximize=FALSE)
  Sys.time() - tme
  bagPreds[,i] <- xgb5cv$pred
  stopRounds[i,"round"] <- which.min(xgb5cv$dt$test.logloss.mean)
  write_csv(stopRounds, "./stack_models/stopRounds_xgb5cv.csv")
  
  #Move the CSV to inside the loop in case there's a crash midway through - Can continue loop from where it left off
  cvPreds <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==0], xgb5cv_pred=bagPreds)
  colnames(cvPreds) <- c("ID", paste0("cvPreds_xgb5_mod",1:models))
  write.csv(cvPreds, "./stack_models/cvPreds/cvPreds_xgb5.csv", row.names=FALSE) 
...
for (i in 1:nrow(grid_samp)) {
  ...
  bagPreds_test[,i] <- predict(xgb5, newdata=data.matrix(ts1Trans[filter==2, c(varnames),with=FALSE]))
  
  submission <- data.frame(ID=sub$ID, PredictedProb=bagPreds_test)
  colnames(submission) <- c("ID", paste0("testPreds_xgb5_mod",1:models))
  write.csv(submission, "./stack_models/testPreds/testPreds_xgb5.csv", row.names=FALSE)
}

submission2 <- data.frame(ID=sub$ID, PredictedProb=rowMeans(bagPreds_test))
write.csv(submission2, "./stack_models/testPreds/testPreds_means/testPreds_xgb5_means.csv", row.names=FALSE)
