...
temp <- c("cvPreds_glmnet1.csv","cvPreds_glmnet2.csv","cvPreds_glmnet3.csv","cvPreds_nn1.csv" ,"cvPreds_xgb17.csv","cvPreds_xgb18.csv","cvPreds_xgb19.csv","cvPreds_xgb20.csv","cvPreds_xgb21.csv")
...
temp <- c("testPreds_glmnet1.csv","testPreds_glmnet2.csv","testPreds_glmnet3.csv","testPreds_nn1.csv" ,"testPreds_xgb17.csv","testPreds_xgb18.csv","testPreds_xgb19.csv","testPreds_xgb20.csv","testPreds_xgb21.csv")
...
Sys.time() - tme
save(xgbLay2_v9, file="./stack_models/xgbLay2_v9.rda")
cvPreds <- data.frame(ID=t1$ID, PredictProb=xgbLay2_v9$pred)
write_csv(cvPreds, "./stack_models/layer2Preds/cvPreds_xgbLay2_v9.csv")
rounds <- floor(which.min(xgbLay2_v9$dt$test.logloss.mean) * 1.08)

# Train on all observations
set.seed(201601)
(tme <- Sys.time())
xgbLay2_v9_full <- xgb.train(data = dtrain,
                        params = param,
                        nrounds = rounds,
                        maximize=FALSE,
                        # watchlist=watchlist,
                        print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v9_full, file="./stack_models/xgbLay2_v9_full.rda")

lay2preds <- predict(xgbLay2_v9_full, newdata=data.matrix(lay1fullpreds))
samp <- read_csv('sample_submission.csv')
submission <- data.frame(ID=sort(samp$ID), PredictedProb=lay2preds)
write.csv(submission, "./stack_models/layer2Preds/xgbLay2_v9preds.csv", row.names=FALSE)

xgbLay2_v9_Imp <- xgb.importance(feature_names = colnames(lay1preds[,2:ncol(lay1preds)]), model=xgbLay2_v9_full)
write_csv(xgbLay2_v9_Imp, "./stack_models/xgbLay2_v9_Imp.csv")
