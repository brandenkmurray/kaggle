...
ts1Trans <- data.table(read.csv("./data_trans/ts2Trans_v17.csv"))
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb22.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb22cv, file="./stack_models/xgb22cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb22cv$pred), "./stack_models/cvPreds/cvPreds_xgb22.csv", row.names=FALSE)

minLossRound <- which.min(xgb22cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb22full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb22full, file="./stack_models/xgb22full.rda")

preds <- predict(xgb22full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb22.csv", row.names=FALSE)


xgb22Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==0, c(varnames), with=FALSE]), model=xgb22full)
write.csv(xgb22Imp, "./stack_models/xgb22Imp.csv", row.names=FALSE)
