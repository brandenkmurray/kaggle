...
# write.csv(tsne_Y, "./stack_models/tsne_xgb13.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb13cv, file="./stack_models/xgb13cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb13cv$pred), "./stack_models/cvPreds/cvPreds_xgb13.csv", row.names=FALSE)

minLossRound <- which.min(xgb13cv$dt$test.logloss.mean)
...
preds <- predict(xgb13full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
submission <- data.frame(ID=ts1Trans$ID[ts1Trans$filter==2], PredictedProb=preds)
write.csv(submission, "./stack_models/testPreds/testPreds_xgb13.csv", row.names=FALSE)


xgb13Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==2, c(varnames), with=FALSE]), model=xgb13full)
write.csv(xgb13Imp, "./stack_models/xgb13Imp.csv", row.names=FALSE)
