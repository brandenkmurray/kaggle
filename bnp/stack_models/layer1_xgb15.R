...
ts1Trans <- data.table(read.csv("./data_trans/ts2Trans_v13.csv"))
...
# write.csv(tsne_Y, "./stack_models/tsne_xgb16.csv", row.names=FALSE)
...
Sys.time() - tme
save(xgb16cv, file="./stack_models/xgb16cv.rda")

write.csv(data.frame(ID=ts1Trans[filter==0,"ID",with=FALSE], PredictedProb=xgb16cv$pred), "./stack_models/cvPreds/cvPreds_xgb16.csv", row.names=FALSE)

minLossRound <- which.min(xgb16cv$dt$test.logloss.mean)
...
set.seed(201512)
(tme <- Sys.time())
xgb16full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb16full, file="./stack_models/xgb16full.rda")

preds <- predict(xgb16full, data.matrix(ts1Trans[filter==2, c(varnames), with=FALSE]))
...
xgb16Imp <- xgb.importance(feature_names = colnames(ts1Trans[filter==2, c(varnames), with=FALSE]), model=xgb16full)
write.csv(xgb16Imp, "./stack_models/xgb16Imp.csv", row.names=FALSE)
