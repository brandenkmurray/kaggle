...
sess_transform <- function(data){
  x <- data[, list(n=length(secs_elapsed),
                   ...
                   actionDetEng = entropy(action_detail),
                   deviceEnt = entropy(device_type)
                   # actionEnt2 = entropy2(action),
#                    actionTypeEnt2 = entropy2(action_type),
#                    actionDetEng2 = entropy2(action_detail),
#                    deviceEnt2 = entropy2(device_type)
  ), by=list(user_id)]
...
write_csv(ts1_pp, "./data_trans/ts1_pp_v1.csv")
