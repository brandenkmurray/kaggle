...
#                 scansUpcRatio=netScans/uniqUpc
  ...
  xActionDet <- dcast.data.table(user_id~action_detail, value.var="secs_elapsed",
                                  fun.aggregate = length, data=data)
  # xActionDet <- data.table(xAction[,"user_id",with=FALSE], prop.table(as.matrix(xAction[,2:ncol(xAction), with=FALSE]),margin=1))
...
pp <- preProcess(ts1_merge, method = c("bagImpute"))
...
write_csv(ts1_pp, "./data_trans/ts1_pp_v6.csv")
