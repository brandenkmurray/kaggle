...
dtrain <- xgb.DMatrix(data=Matrix(data.matrix(ts1Trans[filter==0, xgbImpVars$Feature, with=FALSE])),label=ts1Trans$class[ts1Trans$filter==0])
...
Sys.time() - tme
save(xgb8, file="./stack_models/xgb8.rda")

#

cvPreds <- xgb8$pred
classMap <- read_csv("./data_trans/classMap.csv")
cnames <- paste("xgb8", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb8.csv", row.names=FALSE) 


rounds <- floor(which.min(xgb8$dt$test.mlogloss.mean) * 1.15)

(tme <- Sys.time())
set.seed(201512)
xgb8full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb8full, file="./stack_models/xgb8full.rda")


testPreds <- predict(xgb8full, data.matrix(ts1Trans[filter==2,xgbImpVars$Feature, with=FALSE]))
testPreds <- as.data.frame(matrix(testPreds, nrow=12))
classMap <- read_csv("./data_trans/classMap.csv")
rownames(testPreds) <- classMap$country_destination
write.csv(data.frame(id=ts1Trans$id[ts1Trans$filter==2], t(testPreds)), "./stack_models/testPredsProbs_xgb8.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/testPreds_xgb8.csv", quote=FALSE, row.names = FALSE)

xgb8Imp <- xgb.importance(feature_names=colnames(ts1Trans[filter==2,xgbImpVars$Feature, with=FALSE]), model=xgb8full)
write_csv(xgb8Imp, "./stack_models/xgb8Imp.csv")
