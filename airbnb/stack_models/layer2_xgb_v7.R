...
xgbLay2_v7$dt[which.min(xgbLay2_v7$dt$test.ndcg5.mean),]
rounds <- floor(which.max(xgbLay2_v7$dt$test.ndcg5.mean) * 1.15)
...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v7_full <- xgb.train(data = dtrain,
                              params = param,
                              nrounds = rounds,
                              maximize=TRUE,
                              # watchlist=watchlist,
                              print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v7_full, file="./stack_models/xgbLay2_v7_full.rda")

lay2Preds <- predict(xgbLay2_v7_full, newdata=data.matrix(lay1fullpreds))
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v7.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v7.csv", quote=FALSE, row.names = FALSE)
