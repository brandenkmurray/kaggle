...
glmnet1cv$id <- NULL
rf1cv$id <- NULL
lay1preds <- cbind(xgb1cv, xgb4cv, xgb6cv, xgb7cv, xgb8cv, xgb9cv, xgb10cv, xgb11cv, xgb12cv, xgb13cv, xgb14cv, nn1cv, nn2cv, nn3cv, glmnet1cv, rf1cv)
...
xgbLay2_v9$dt[which.min(xgbLay2_v9$dt$test.ndcg5.mean),]
rounds <- floor(which.max(xgbLay2_v9$dt$test.ndcg5.mean) * 1.15)
...
lay1fullpreds <- cbind(xgb1fullpreds,xgb4fullpreds,xgb6fullpreds, xgb7fullpreds, xgb8fullpreds, xgb9fullpreds, xgb10fullpreds, xgb11fullpreds, xgb12fullpreds, xgb13fullpreds, xgb14fullpreds, nn1fullpreds, nn2fullpreds,nn3fullpreds,glmnet1fullpreds,rf1fullpreds)
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v9.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v9.csv", quote=FALSE, row.names = FALSE)
