...
cnames <- paste("xgb2", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb2.csv", row.names=FALSE) 

rounds <- floor(which.min(xgb2$dt$test.mlogloss.mean) * 1.2)
...
testPreds <- predict(xgb2full, data.matrix(ts1Trans[filter==2,varnames, with=FALSE]))
testPreds <- as.data.frame(matrix(testPreds, nrow=12))
classMap <- read_csv("./data_trans/classMap.csv")
rownames(testPreds) <- classMap$country_destination
write.csv(data.frame(id=ts1Trans$id[ts1Trans$filter==2], t(testPreds)), "./stack_models/testPredsProbs_xgb2.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/testPreds_xgb2.csv", quote=FALSE, row.names = FALSE)
