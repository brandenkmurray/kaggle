...
cnames <- paste("xgb10", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb10.csv", row.names=FALSE) 


rounds <- floor(which.min(xgb10$dt$test.mlogloss.mean) * 1.15)
...
testPreds <- predict(xgb10full, data.matrix(ts1Trans[filter==2,vars, with=FALSE]))
testPreds <- as.data.frame(matrix(testPreds, nrow=12))
classMap <- read_csv("./data_trans/classMap.csv")
rownames(testPreds) <- classMap$country_destination
write.csv(data.frame(id=ts1Trans$id[ts1Trans$filter==2], t(testPreds)), "./stack_models/testPredsProbs_xgb10.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/testPreds_xgb10.csv", quote=FALSE, row.names = FALSE)

xgb10Imp <- xgb.importance(feature_names=colnames(ts1Trans[filter==0,vars, with=FALSE]), model=xgb10full)
write_csv(xgb10Imp, "./stack_models/xgb10Imp.csv")
