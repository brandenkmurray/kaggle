...
Sys.time() - tme
save(xgb19, file="./stack_models/xgb19.rda")

cvPreds <- xgb19$pred
classMap <- read_csv("./data_trans/classMap.csv")
cnames <- paste("xgb19", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb19.csv", row.names=FALSE) 


rounds <- floor(which.max(xgb19$dt$test.ndcg5.mean) * 1.2)

(tme <- Sys.time())
set.seed(201512)
xgb19full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb19full, file="./stack_models/xgb19full.rda")


testPreds <- predict(xgb19full, data.matrix(ts1Trans[filter==2,c("daysDiffCreatedAct",xgbImpVars$Feature), with=FALSE]))
testPreds <- as.data.frame(matrix(testPreds, nrow=12))
classMap <- read_csv("./data_trans/classMap.csv")
rownames(testPreds) <- classMap$country_destination
write.csv(data.frame(id=ts1Trans$id[ts1Trans$filter==2], t(testPreds)), "./stack_models/testPredsProbs_xgb19.csv", row.names=FALSE)
