...
weights <- (ts1Trans$activeYear.Mon[ts1Trans$filter==0] - 2009) / sum(ts1Trans$activeYear.Mon[ts1Trans$filter==0] - 2009) * 10000

dtrain <- xgb.DMatrix(data=Matrix(data.matrix(ts1Trans[filter==0,varnames, with=FALSE])),label=ts1Trans$class[ts1Trans$filter==0], weight=weights)
...
Sys.time() - tme
save(xgb18, file="./stack_models/xgb18.rda")

cvPreds <- xgb18$pred
classMap <- read_csv("./data_trans/classMap.csv")
cnames <- paste("xgb18", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb18.csv", row.names=FALSE) 


rounds <- floor(which.min(xgb18$dt$test.mlogloss.mean) * 1.2)
...
testPreds <- predict(xgb18full, data.matrix(ts1Trans[filter==2,c("daysDiffCreatedAct",xgbImpVars$Feature), with=FALSE]))
testPreds <- as.data.frame(matrix(testPreds, nrow=12))
classMap <- read_csv("./data_trans/classMap.csv")
rownames(testPreds) <- classMap$country_destination
write.csv(data.frame(id=ts1Trans$id[ts1Trans$filter==2], t(testPreds)), "./stack_models/testPredsProbs_xgb18.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/testPreds_xgb18.csv", quote=FALSE, row.names = FALSE)
