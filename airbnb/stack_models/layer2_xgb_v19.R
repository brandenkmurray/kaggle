...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v19 <- xgb.cv(data = dtrain,
                      params = param,
                      nrounds = 10000,
                      # feval = ndcg5,
                      maximize=TRUE,
...
Sys.time() - tme
save(xgbLay2_v19, file="./stack_models/xgbLay2_v19.rda")

xgbLay2_v19$dt[which.max(xgbLay2_v19$dt$test.ndcg5.mean),]
rounds <- floor(which.max(xgbLay2_v19$dt$test.ndcg5.mean) * 1.15)
...
lay1fullpreds <- cbind(xgb4fullpreds,xgb5fullpreds,xgb6fullpreds, xgb14fullpreds, xgb15fullpreds, xgb17fullpreds, xgb18fullpreds, xgb19fullpreds, nn1fullpreds, nn2fullpreds,nn3fullpreds, nn4fullpreds, glmnet1fullpreds,rf1fullpreds)
# Predict the test set using the XGBOOST stacked model
colnames(lay1fullpreds) <- colnames(lay1preds[,2:ncol(lay1preds)])


set.seed(201510)
(tme <- Sys.time())
xgbLay2_v19_full <- xgb.train(data = dtrain,
                              params = param,
                              nrounds = rounds,
                              maximize=TRUE,
                              # watchlist=watchlist,
                              print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v19_full, file="./stack_models/xgbLay2_v19_full.rda")

lay2Preds <- predict(xgbLay2_v19_full, newdata=data.matrix(lay1fullpreds))
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v19.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v19.csv", quote=FALSE, row.names = FALSE)

xgbLay2v19Imp <- xgb.importance(feature_names = colnames(lay1fullpreds), model=xgbLay2_v19_full )
write_csv(xgbLay2v19Imp, "./stack_models/xgbLay2v19Imp.csv")
