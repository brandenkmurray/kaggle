...
xgbLay2_v2$dt[which.min(xgbLay2_v2$dt$test.mlogloss.mean),]
rounds <- floor(which.min(xgbLay2_v2$dt$test.mlogloss.mean) * 1.15)
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v2.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v2.csv", quote=FALSE, row.names = FALSE)
