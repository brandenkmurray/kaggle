...
stopCluster(cl)
Sys.time() - tme
save(rf1, file="./stack_models/layer1_rf1.rda")

cvPreds <- rf1$pred[,3:15]
...
cnames <- paste("rf1", names(cvPreds)[1:ncol(cvPreds)], sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_rf1.csv", row.names=FALSE) 


preds <- predict(rf1, newdata=test, type="prob")
preds <- preds[,mixedorder(names(preds))]
samp <- read_csv('sample_submission_NDF.csv')
classMap <- read_csv("./data_trans/classMap.csv")
colnames(preds) <- paste("rf1",classMap$country_destination,sep="_")
sampID <- read_csv('sample_submission_NDF.csv')$id
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, preds), "./stack_models/testPredsProbs_rf1.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/testPreds_rf1.csv", quote=FALSE, row.names = FALSE)
