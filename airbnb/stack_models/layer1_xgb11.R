...
ts1Trans <- data.table(read.csv("./data_trans/ts1_pp_v9.csv"))
...
Sys.time() - tme
save(xgb11, file="./stack_models/xgb11.rda")

#

cvPreds <- xgb11$pred
classMap <- read_csv("./data_trans/classMap.csv")
cnames <- paste("xgb11", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb11.csv", row.names=FALSE) 


rounds <- floor(which.min(xgb11$dt$test.mlogloss.mean) * 1.15)

(tme <- Sys.time())
set.seed(201512)
xgb11full <- xgb.train(data = dtrain,
                      params = param,
                      nrounds = rounds,
                      maximize=FALSE,
                      print.every.n = 20)
Sys.time() - tme
save(xgb11full, file="./stack_models/xgb11full.rda")


testPreds <- predict(xgb11full, data.matrix(ts1Trans[filter==2, xgbImpVars$Feature, with=FALSE]))
...
write.csv(submission, "./stack_models/testPreds_xgb11.csv", quote=FALSE, row.names = FALSE)

# xgb11Imp <- xgb.importance(feature_names=colnames(ts1Trans[filter==0, xgbImpVars$Feature, with=FALSE]), model=xgb11full)
# write_csv(xgb11Imp, "./stack_models/xgb11Imp.csv")
