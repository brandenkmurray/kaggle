...
ts1Trans = pd.read_csv("/home/branden/Documents/kaggle/airbnb/data_trans/ts1_pp_v4.csv") 
...
num_features = t1nn_pca.shape[1]


import theano
t1nn_pca_shared = theano.shared(t1nn_pca, borrow=True)
...
#                     EarlyStopping(patience=200),
                 ...
                 verbose=1,
                 batch_iterator_train=BatchIterator(400),
                 max_epochs=680)

#np.random.seed(15)
#net0_clone = clone(net0)
#net0_clone.fit(t1nn_pca_shared.get_value(), y)
...
foldPred = np.zeros((t1nn_pca_shared.get_value().shape[0], num_classes))
bags = 20
...
for iter in xrange(0,bags):
        for fold in xrange(0,folds):
            ...
            net0_clone = clone(net0)
            net0_clone.fit(t1nn_pca_shared.get_value()[cv_by_hand[fold][0],:], y[cv_by_hand[fold][0]])
            foldPred[cv_by_hand[fold][1],:] += net0_clone.predict_proba(t1nn_pca_shared.get_value()[cv_by_hand[fold][1],:])
...
# Save 
foldPredFrame.to_csv("/home/branden/Documents/kaggle/airbnb/stack_models/cvPreds_nn1.csv", index=False)
...
test_pred= np.zeros((s1nn_pca.shape[0], num_classes))
for iter in xrange(0,bags):
    np.random.seed(iter + 59)
    net1_clone = clone(net1)
    net1_clone.fit(t1nn_pca_shared.get_value(), y)
    test_pred += net1_clone.predict_proba(s1nn_pca)
...
## Save 
#test_pred_frame.to_csv("/home/branden/Documents/kaggle/airbnb/stack_models/testPredsProbs_nn1.csv", index=False)
...
# Save 
testPredFrame.to_csv("/home/branden/Documents/kaggle/airbnb/stack_models/testPredsProbs_nn1.csv", index=False)
