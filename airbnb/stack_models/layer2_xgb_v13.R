...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v13 <- xgb.cv(data = dtrain,
                      params = param,
                      nrounds = 10000,
                      # feval = ndcg5,
                      maximize=TRUE,
...
Sys.time() - tme
save(xgbLay2_v13, file="./stack_models/xgbLay2_v13.rda")

xgbLay2_v13$dt[which.min(xgbLay2_v13$dt$test.ndcg5.mean),]
rounds <- floor(which.max(xgbLay2_v13$dt$test.ndcg5.mean) * 1.15)
...
lay1fullpreds <- cbind(xgb1fullpreds, xgb2fullpreds, xgb4fullpreds,xgb6fullpreds, xgb7fullpreds, xgb8fullpreds, xgb9fullpreds, xgb10fullpreds, xgb11fullpreds, xgb12fullpreds, xgb13fullpreds, xgb14fullpreds, nn1fullpreds, nn2fullpreds,nn3fullpreds, nn4fullpreds, glmnet1fullpreds,rf1fullpreds)
# Predict the test set using the XGBOOST stacked model

set.seed(201510)
(tme <- Sys.time())
xgbLay2_v13_full <- xgb.train(data = dtrain,
                              params = param,
                              nrounds = rounds,
                              maximize=TRUE,
                              # watchlist=watchlist,
                              print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v13_full, file="./stack_models/xgbLay2_v13_full.rda")

lay2Preds <- predict(xgbLay2_v13_full, newdata=data.matrix(lay1fullpreds))
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v13.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v13.csv", quote=FALSE, row.names = FALSE)
