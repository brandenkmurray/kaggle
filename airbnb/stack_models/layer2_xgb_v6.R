...
glmnet1cv$id <- NULL
rf1cv$id <- NULL
lay1preds <- cbind(xgb6cv, xgb7cv, xgb8cv, xgb9cv, xgb10cv, xgb11cv, xgb12cv, xgb13cv, nn1cv, nn2cv, nn3cv, glmnet1cv, rf1cv)
...
xgbLay2_v6 <- xgb.cv(data = dtrain,
                      ...
                      early.stop.round=50)
Sys.time() - tme
save(xgbLay2_v6, file="./stack_models/xgbLay2_v6.rda")

xgbLay2_v6$dt[which.min(xgbLay2_v6$dt$test.mlogloss.mean),]
rounds <- floor(which.min(xgbLay2_v6$dt$test.mlogloss.mean) * 1.15)
...
lay1fullpreds <- cbind(xgb6fullpreds, xgb7fullpreds, xgb8fullpreds, xgb9fullpreds, xgb10fullpreds, xgb11fullpreds, xgb12fullpreds, xgb13fullpreds, nn1fullpreds, nn2fullpreds,nn3fullpreds,glmnet1fullpreds,rf1fullpreds)
# Predict the test set using the XGBOOST stacked model

set.seed(201510)
(tme <- Sys.time())
xgbLay2_v6_full <- xgb.train(data = dtrain,
                              params = param,
                              nrounds = rounds,
                              maximize=FALSE,
                              # watchlist=watchlist,
                              print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v6_full, file="./stack_models/xgbLay2_v6_full.rda")

lay2Preds <- predict(xgbLay2_v6_full, newdata=data.matrix(lay1fullpreds))
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v6.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v6.csv", quote=FALSE, row.names = FALSE)
