...
xgbLay2_v11$dt[which.min(xgbLay2_v11$dt$test.ndcg5.mean),]
rounds <- floor(which.max(xgbLay2_v11$dt$test.ndcg5.mean) * 1.15)
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v11.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v11.csv", quote=FALSE, row.names = FALSE)
