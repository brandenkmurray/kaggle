...
glmnet1cv$id <- NULL
rf1cv$id <- NULL
lay1preds <- cbind(xgb4cv, xgb6cv, xgb8cv, xgb9cv, xgb10cv, xgb11cv, xgb12cv, xgb13cv, xgb14cv, nn1cv, nn2cv, nn3cv, nn4cv, glmnet1cv, rf1cv)
# Get actual classes
...
xgbLay2_v15$dt[which.min(xgbLay2_v15$dt$test.ndcg5.mean),]
rounds <- floor(which.max(xgbLay2_v15$dt$test.ndcg5.mean) * 1.15)
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v15.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v15.csv", quote=FALSE, row.names = FALSE)
