...
nn1cv$id <- NULL
glmnet1cv$id <- NULL
lay1preds <- cbind(xgb1cv, xgb2cv, xgb3cv, xgb4cv, xgb5cv, xgb6cv, xgb7cv, xgb8cv,  nn1cv, glmnet1cv)
...
xgbLay2_v4$dt[which.min(xgbLay2_v4$dt$test.mlogloss.mean),]
rounds <- floor(which.min(xgbLay2_v4$dt$test.mlogloss.mean) * 1.15)
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v4.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v4.csv", quote=FALSE, row.names = FALSE)
