...
glmnet1cv$id <- NULL
rf1cv$id <- NULL
lay1preds <- cbind(xgb1cv, xgb2cv, xgb4cv, xgb6cv, xgb7cv, xgb8cv, xgb9cv, xgb10cv, xgb11cv, xgb12cv, xgb13cv, xgb14cv, xgb15cv, nn1cv, nn2cv, nn3cv, nn4cv, glmnet1cv, rf1cv)
# Get actual classes
...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v14 <- xgb.cv(data = dtrain,
                      params = param,
                      nrounds = 10000,
                      # feval = ndcg5,
                      maximize=TRUE,
...
Sys.time() - tme
save(xgbLay2_v14, file="./stack_models/xgbLay2_v14.rda")

xgbLay2_v14$dt[which.min(xgbLay2_v14$dt$test.ndcg5.mean),]
rounds <- floor(which.max(xgbLay2_v14$dt$test.ndcg5.mean) * 1.15)
...
lay1fullpreds <- cbind(xgb1fullpreds, xgb2fullpreds, xgb4fullpreds,xgb6fullpreds, xgb7fullpreds, xgb8fullpreds, xgb9fullpreds, xgb10fullpreds, xgb11fullpreds, xgb12fullpreds, xgb13fullpreds, xgb14fullpreds, xgb15fullpreds, nn1fullpreds, nn2fullpreds,nn3fullpreds, nn4fullpreds, glmnet1fullpreds,rf1fullpreds)
# Predict the test set using the XGBOOST stacked model

set.seed(201510)
(tme <- Sys.time())
xgbLay2_v14_full <- xgb.train(data = dtrain,
                              params = param,
                              nrounds = rounds,
                              maximize=TRUE,
                              # watchlist=watchlist,
                              print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v14_full, file="./stack_models/xgbLay2_v14_full.rda")

lay2Preds <- predict(xgbLay2_v14_full, newdata=data.matrix(lay1fullpreds))
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v14.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v14.csv", quote=FALSE, row.names = FALSE)
