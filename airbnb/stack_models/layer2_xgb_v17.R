...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v17 <- xgb.cv(data = dtrain,
                      params = param,
                      nrounds = 10000,
                      # feval = ndcg5,
                      maximize=FALSE,
...
Sys.time() - tme
save(xgbLay2_v17, file="./stack_models/xgbLay2_v17.rda")

xgbLay2_v17$dt[which.min(xgbLay2_v17$dt$test.mlogloss.mean),]
rounds <- floor(which.min(xgbLay2_v17$dt$test.mlogloss.mean) * 1.15)
...
set.seed(201510)
(tme <- Sys.time())
xgbLay2_v17_full <- xgb.train(data = dtrain,
                              params = param,
                              nrounds = rounds,
                              maximize=TRUE,
                              # watchlist=watchlist,
                              print.every.n = 5)
Sys.time() - tme
save(xgbLay2_v17_full, file="./stack_models/xgbLay2_v17_full.rda")

lay2Preds <- predict(xgbLay2_v17_full, newdata=data.matrix(lay1fullpreds))
...
sampID <- sort(sampID)
write.csv(data.frame(id=sampID, t(lay2Preds)), "./stack_models/lay2PredsProbs_xgb_v17.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/lay2Preds_xgb_v17.csv", quote=FALSE, row.names = FALSE)

xgbLay2v17Imp <- xgb.importance(feature_names = colnames(lay1preds[,2:ncol(lay1preds), with=FALSE]), model=xgbLay2_v17_full )
