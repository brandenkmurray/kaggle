...
kknn1_stack_preds <- matrix(0, nrow=nrow(t1kknn), ncol=12)
...
for (i in 1:length(cvFoldsList)){
  kknn1 <- kknn(class ~ .,
                ...
                distance=1,
                kernel="biweight")
  kknn1_stack_preds[cvFoldsList[[1]],] <- kknn1$prob
  logLossTable[i,2] <- LogLoss(model.matrix(~class-1, t1kknn[cvFoldsList[[1]],"class", with=FALSE]), kknn1$prob)
}
