...
xgb5test <- read_csv("./stack_models/testPredsProbs_xgb5.csv") 
xgb6test <- read_csv("./stack_models/testPredsProbs_xgb6.csv")
...
blendFrame <- (xgb5cv[,2:ncol(xgb5cv)]*.9 + xgb6cv[,2:ncol(xgb6cv)]*.1)
ndcg5_v2(blendFrame, t1sub$class)

blendFrameTest <- (lay2_xgb9_test[,2:ncol(lay2_xgb9_test)] + lay2_nn2_test[,2:ncol(lay2_nn2_test)])/2
blendFrameTest <- (xgb6test[,2:ncol(xgb6test)]*.9 + xgb5test[,2:ncol(xgb5test)]*.1)
...
write.csv(submission, "./stack_models/blended_v2.csv", quote=FALSE, row.names = FALSE)
