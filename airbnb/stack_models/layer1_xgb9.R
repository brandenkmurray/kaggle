...
ts1Trans <- data.table(read.csv("./data_trans/ts1_pp_v5.csv"))
...
cnames <- paste("xgb9", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb9.csv", row.names=FALSE) 


rounds <- floor(which.min(xgb9$dt$test.mlogloss.mean) * 1.15)
...
testPreds <- predict(xgb9full, data.matrix(ts1Trans[filter==2,vars, with=FALSE]))
testPreds <- as.data.frame(matrix(testPreds, nrow=12))
classMap <- read_csv("./data_trans/classMap.csv")
rownames(testPreds) <- classMap$country_destination
write.csv(data.frame(id=ts1Trans$id[ts1Trans$filter==2], t(testPreds)), "./stack_models/testPredsProbs_xgb9.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/testPreds_xgb9.csv", quote=FALSE, row.names = FALSE)

xgb9Imp <- xgb.importance(feature_names=colnames(ts1Trans[filter==0,vars, with=FALSE]), model=xgb9full)
write_csv(xgb9Imp, "./stack_models/xgb9Imp.csv")
