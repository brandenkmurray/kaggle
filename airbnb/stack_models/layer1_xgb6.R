...
Sys.time() - tme
save(xgb6, file="./stack_models/xgb6.rda")

cvPreds <- xgb6$pred
classMap <- read_csv("./data_trans/classMap.csv")
cnames <- paste("xgb6", classMap$country_destination, sep="_")
colnames(cvPreds) <- cnames
write.csv(data.frame(id=ts1Trans[filter==0,"id",with=FALSE], cvPreds), "./stack_models/cvPreds_xgb6.csv", row.names=FALSE) 


rounds <- floor(which.min(xgb6$dt$test.mlogloss.mean) * 1.2)
...
testPreds <- predict(xgb6full, data.matrix(ts1Trans[filter==2,c("daysDiffCreatedAct",xgbImpVars$Feature), with=FALSE]))
testPreds <- as.data.frame(matrix(testPreds, nrow=12))
classMap <- read_csv("./data_trans/classMap.csv")
rownames(testPreds) <- classMap$country_destination
write.csv(data.frame(id=ts1Trans$id[ts1Trans$filter==2], t(testPreds)), "./stack_models/testPredsProbs_xgb6.csv", row.names=FALSE)
...
write.csv(submission, "./stack_models/testPreds_xgb6.csv", quote=FALSE, row.names = FALSE)
